...
import { ensureLearnerOnboarded, getFixtureIds, getLearnerId, makeVocabularyItemDue, resetLearnerItemProgress } from "../support/e2e-state";
...
test.describe("Review -> Progress", () => {
  ...
  test("dashboard shows a due review, completing it persists progress, and it stops appearing as due", async ({ page }) => {
    ...
    expect(progress!.nextReviewAt && progress!.nextReviewAt.getTime() > Date.now()).toBe(true);
