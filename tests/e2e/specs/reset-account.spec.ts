...
test.describe("Reset Entire Account", () => {
  ...
  test("resets progress and onboarding, returning the learner to onboarding", async ({
    page,
  }) => {
    ...
    await clickRespectingDangerZoneRateLimit(
      page,
      page.getByRole("dialog").getByRole("button", { name: "Reset Account" }),
    );

    await expect(page).toHaveURL(/\/onboarding/, { timeout: 15_000 });

    const [user] = await withE2EDb((db) =>
      db.select().from(users).where(eq(users.id, learnerId)),
    );
