...
- **Should production Neon be a branch of the same Neon project dev/preview use, or a fully separate Neon project?** (spec 23, 2026-09-16). Architecturally either satisfies "separate resources for development/preview/production," but a separate _project_ means the `NEON_API_KEY` given to every PR-triggered CI workflow (`ci.yml`, `migrate.yml`, `e2e.yml`) has zero API-level reach into production even in principle, where a same-project branch relies entirely on that key being scoped narrowly (Neon's project-scoped API keys) and nobody ever widening it later. No code depends on the answer — this only affects the runbook's step 9 (Neon runbook) and which Neon dashboard the owner provisions from. Resolve before creating the real production database.
