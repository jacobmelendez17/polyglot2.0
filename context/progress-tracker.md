...
**Most recent unit (2026-09-23): the Practice hub (`/practice`) — frontend plus backend foundation, from the user-supplied `practice-hub` wireframe.** Shipped below under Completed; the open product questions it surfaced are under Open Questions ("Practice hub"). The spec 24 unit that follows is still the standing goal.
...
- **Fix: Ghost Reviews sentence lookup was an unbounded N+1 query (2026-09-23)** — user asked why render times felt slow; found while auditing the lessons/reviews/dashboard hot paths for query waterfalls (the others — `lesson-service.ts`, `review-queue.ts`, `dashboard-aggregation.ts` — are pure in-memory loops, no DB calls inside). `buildGhostReviewViews` in `domains/srs/review-orchestration.ts`, run on every `/reviews` page load, already batched its learning-item lookups (`getLearningItemsByIds`) but looked up each due Ghost's sentence one at a time with a sequential `await getSentenceById(...)` inside the loop — and `getDueGhosts` (`domains/srs/ghost-repository.ts`) has no `LIMIT`, so the round-trip count scales with however many Ghosts a learner has accumulated. Added `getSentencesByIds` (`domains/curriculum/curriculum-repository.ts`, batched via `inArray`, mirroring the existing item-batching pattern) and switched the loop to a `Map` lookup fed by one `Promise.all`-parallel query pair. `ghost-orchestration.ts`'s `submitGhostAnswer` was already fine (single ghost, already `Promise.all`'d) and untouched. Verified `tsc`, and the full `npm run test:integration` slice for this domain (`review-orchestration`, `ghost-repository`, `ghost-orchestration` — 59/59) against the real test branch.

- **Admin curriculum list/editor UX fixes (2026-09-23, user-reported)** — four related complaints about `/admin/curriculum`:
  - **Whole row is now clickable**, not just a small "Edit" link at the far right. `components/admin/curriculum/curriculum-table.tsx` uses a stretched-link: the item-label cell holds a real `<Link>` with `after:absolute after:inset-0` (matching the existing `after:` pseudo-element convention already used in `components/ui/tabs.tsx`, which needs no explicit `content-['']` in this repo's Tailwind setup), and the `<tr>` is `relative` so the pseudo-element's containing block is the whole row. The checkbox cell keeps `relative z-10` so it stays clickable on top of the overlay rather than being swallowed by it — no `stopPropagation`/JS click-routing needed. The far-right column is now a decorative chevron instead of the old "Edit" text link.
  - **Filters survive a visit to the item editor and back.** The table reads its own current `useSearchParams()` and appends it as `?from=<encoded query string>` on every row's link; `app/(admin)/admin/curriculum/items/[itemId]/page.tsx` reads `from` and builds its new "← Back to Curriculum" link (same visual convention as `imports/[importId]/page.tsx`'s existing back link) from it, falling back to a bare `/admin/curriculum` when absent. The sidebar's "Curriculum" nav link (`components/admin/admin-nav-items.ts`) has no query string at all, so it — deliberately, per the user's own framing — still resets to a blank filtered view; only the new in-page Back link preserves state.
  - **Group filter is now scoped to the selected Level.** `components/admin/curriculum/curriculum-filters.tsx` filters the Group dropdown's options down to the selected level's `levelNumber` (falling back to every group when no level is selected), and drops the redundant `L{n} —` prefix once scoped since the level is already implied. Selecting a Level now also clears any previously-set `groupId` in the same navigation, so a group from a since-deselected level can't linger as an invisible active filter.
  - **Verification**: `tsc`, `eslint` clean on every touched file; updated `curriculum-filters.test.tsx` (one pre-existing test had mismatched level/group fixture data that only passed because groups were never scoped before — fixed the fixture, added two new tests for level-scoped options and group-clears-on-level-change) and `curriculum-table.test.tsx` (mocked `useSearchParams`, updated the row-link assertion, added a `from`-threading test). Full `npm run test`: 164 files / 1103 tests, all passing.

- **Practice hub — `/practice` (2026-09-23, from the user's `practice-hub.html`/`.css` wireframe)** — the hub page only. Every practice it lists is still unbuilt, so each renders as a disabled "Coming soon" control (see Open Questions). Scoped as frontend + the minimum backend to make its numbers real, per the request.
  - **Route** `app/(app)/practice/{page,loading,error}.tsx`, inheriting the `(app)` layout's onboarding/curriculum-choice gates. `proxy.ts` now protects `/practice(.*)`. The header and mobile nav already linked to `/practice` (it was a 404 before); the footer's Product column now links it too, since its own rule is "link only routes that resolve" (`footer.tsx` + its test updated).
  - **`domains/practice` (new, per architecture.md's `practice` boundary)** — `practice-types.ts`, `practice-catalog.ts` (all copy, grouping, walk steps, and each practice's `href` as data), `practice-hub-view.ts` (pure `buildPracticeHubView(activity, now)`, `parsePracticeSkillFilter`, `filterGroves`), `practice-repository.ts` (one aggregating query), `practice-service.ts` (injected `DbClient`, `now` via `resolveUserNow` so sandbox clocks work), `server.ts`/`index.ts` (the same server/client-safe split as `domains/decks`).
  - **Schema/migration `0039_next_dragon_lord`** — `practice_sessions` (user, language, `practice_type` enum, `completed_at`) plus one composite index. Purely additive: new type and new table, no backfill, safe against the previously deployed app. Applied to the dev database; `db:verify` clean. It is an **activity log only** — no SRS or skill-progression value — and **has no writer yet**: each practice's completion flow will insert its own row when it ships, so every grove currently reads "0 in 7 days / Not tried yet". That is the true state, not a placeholder.
  - **Components** `components/practice/*` (Server Components throughout; the skill filter is URL state `?skill=`, plain links with `aria-current`, so there is no client JS on the page). `PracticeAction` is the single place an unbuilt route becomes a disabled "Coming soon" button; shipping a practice is just setting its catalog `href`, and the walk switches on by itself once its runner route and every step exist.
  - **Design tokens** — `--practice-listening/-speaking/-reading/-writing` added to `globals.css` (light + dark) and `ui-context.md` updated (see Architecture Decisions). The wireframe's hard-coded hex values were not carried over: everything else maps onto existing tokens (`--bg-*`, `--accent-primary`, `--srs-master`, and `--learning-grammar` for the Grammar conjugations card, since Grammar is red everywhere else). Hub-only wobble/pinned-note/plank/leaf styling and keyframes live in `globals.css` (selector-driven, so not expressible as utilities), wrapped in `@layer components` so Tailwind utilities can still override them, and stop under `prefers-reduced-motion`.
  - **Deliberately left out of the wireframe, and why** (each would be an inert or invented control): per-practice "Adjust setup" links and the round quick-start buttons (no saved-setup concept exists), the dashed "illustration" placeholder box, the wireframe's sample content ("Level 3 · Module 2 — Ready", "New story: El zorro y el cuervo", "Best 88%" — no tests or stories exist, so the Tests card says so honestly), and the conjugation tense chips' on/off state (nothing persists a selection, so tenses render as plain tags). "Shuffle" and "Start walk" are rendered but disabled with a stated reason.
  - **"N this week" became "N in 7 days"**: a rolling window avoids a calendar-week/timezone rule nobody has specified, and the label now says exactly what is counted. The dots cap at five; the number beside them is exact.
  - **Verification**: `tsc`, `eslint .`, `prettier --check` (touched files), `npm run test` (1130/1130, incl. 31 new across the domain view builder and hub components), `npm run test:integration -- domains/practice` (4/4 against the real test database: exact counts, last-session-from-all-history, and no leakage across learners or languages), `npm run build` (`/practice` compiles), `npm run db:verify`. **Not done: a real-browser visual pass** (Clerk-authenticated Playwright) — the page's styling, dark mode, and mobile layout are unreviewed by eye. Do that before treating the visuals as final.
...
4. `/demo` — linked from `SiteHeader`, currently 404; still not built (a separate, unscheduled `00-demo.md` spec). `/journey` is the same (linked from `AppHeader`/`AppNavMobile`); `/practice` now exists as a hub (2026-09-23) whose practices are all still unbuilt — see item 6. `/about` (spec 21, 2026-09-15), `/reviews` (spec 09), `/levels/[level]` (spec 10), and `/decks` (spec 14, 2026-09-08) are all done now — remove them from this list mentally.
...
6. Custom lesson item selection, deck reviews, and the practice experiences themselves (listening, speaking, stories, sentence practice, journal, conjugation, tests — the `/practice` hub that lists them shipped 2026-09-23; each practice's route, its `practice_sessions` writer, and its catalog `href` are still to do) — spec 07 explicitly scoped these out (§89) and spec 09 explicitly scoped deck reviews/practice out too (§3); the _normal_ SRS review session itself is done (spec 09).
...
- **Practice hub (2026-09-23) — product decisions the wireframe assumed but no context file defines.** None blocked the hub, so each was left unbuilt rather than guessed:
  - **What does "saved setup" contain, per practice?** The wireframe's "Set up", "Adjust setup", and quick-start controls, and "each step uses your saved setup", all presume a per-learner, per-practice configuration (duration, difficulty, content filter…). No such thing is specified, so no table or control exists.
  - **How is "Today's walk" composed, and what does Shuffle do?** The steps are a fixed constant in `practice-catalog.ts` (Listening 5 → Sentences 10 → Journal 5). Whether it should vary by learner, level, or recent activity is undecided.
  - **When does each practice unlock?** `project-overview.md` says practice "unlocks according to curriculum level and learned content" but gives no rule (architecture.md's Level Unlock section only names theme tests after Level 5 and CEFR after Level 10 as examples). The hub currently shows every practice to everyone; when rules exist they belong in configuration, not `if level >= X`.
  - **Does "this week" mean a rolling 7 days or a calendar week in the learner's timezone?** Rolling was used and labelled honestly ("N in 7 days").
  - **Wireframe visual identity vs. `ui-context.md`'s "no unique colors per practice type" and "elaborate graphics need a design decision".** Treated the user-supplied wireframe as that explicit design decision: four practice-skill tokens were added and the falling-leaves/pinned-note styling kept. If the intent was *not* to expand the palette, the tokens are four lines in `globals.css` and easy to collapse onto existing ones.
...
- **An unbuilt destination is data, not a dead link** (2026-09-23) — the hub's practices carry `href: string | null` in `domains/practice/practice-catalog.ts`, and one component (`PracticeAction`, plus `PracticeNote` for pinned cards) turns `null` into a disabled "Coming soon" state. The alternative — linking every card to a route that 404s, or omitting the cards — either breaks the "link only routes that resolve" rule (spec 21) or hides the product's intended shape. Shipping a practice is a one-line catalog change, and derived things (the walk becoming startable) follow automatically.
- **Practice activity is a log; skill progression is not built here** (2026-09-23) — `practice_sessions` records completed sessions only. It is deliberately not where skill stages (architecture.md's Skill Progression) will live: those have configurable, per-practice advancement thresholds and will get their own tables when specified. Keeping the log separate means the hub's counts can never be mistaken for, or drift into, progression state, and practice still cannot touch core SRS (code-standards.md's Practice Standards).
- **Per-skill colors are fixed tokens, like the learning-type colors** (2026-09-23) — `--practice-listening/-speaking/-reading/-writing` do not follow the Appearance Color Palette, and are always paired with a label or icon. Documented in `ui-context.md`.
