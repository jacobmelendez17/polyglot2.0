...
Work through `context/feature-specs/` in order. Specs 01–04 are complete. Next unit is the handwriting "hola" stroke animation deferred by spec 03.
...
3. `/about`, `/demo` — linked from the navbar, currently 404.
4. Neon-backed internal user record + server-authoritative role resolution (the `users`/`auth` domain plumbing `architecture.md` describes). Deliberately deferred out of spec 04 — no database layer exists yet. `proxy.ts` has nothing to protect until this and a first authenticated page (e.g. `/dashboard`) exist.
...
- No browser-verification skill exists for this repo. Specs 03 and 04 each installed Playwright ad hoc into a scratch directory, not the project lockfile. Consider capturing this as a project skill (`/run-skill-generator`) if browser checks become routine.
