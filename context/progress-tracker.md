...
- **Wired into the flagship/shared-boundary call sites** (each verified against its own real integration test where one exists, not just typecheck):
  - **`domains/idempotency/with-idempotency.ts`** — `idempotency.new`/`.replayed`/`.in_progress`/`.failed` (mapped: lock-timeout and in-progress-conflict → `.in_progress`; payload mismatch → `.failed`). This is the shared transaction boundary underneath *every* progress-affecting mutation in the app (review completion, lesson completion, and every future one), so this one change gives all of them idempotency-lifecycle visibility for free. 46/47 `with-idempotency.integration.test.ts` passed — the one failure is Next Up #32's already-documented, pre-existing, unrelated cleanup-count flake (accumulated committed rows on the shared `polyglot-test` branch), reproduced independent of this change.
...
- **Bug-fix sweep through the Next Up / Open Questions backlog** (2026-09-17,
  ...
  - **`¿cómo estás?` fix applied**: `deriveDictionaryLookups` now also
    ...
    what that surfaced. **Not yet committed for real against `DATABASE_URL`**
    — what the dry run found is much bigger than the one item originally in
    scope and needs another explicit go-ahead; see chat.
...
27. **Investigated 2026-09-17 (per user request), not yet implemented — see chat for the options presented.** `drizzle-orm`'s schema DSL does support this: `index("name").on(col).concurrently()` exists (`pg-core/indexes.ts`) and `drizzle-kit generate` (confirmed installed: drizzle-kit 0.31.10, drizzle-orm 0.45.2) correctly emits `CREATE INDEX CONCURRENTLY ...` for it. But `db:migrate`'s actual execution path never can run that statement successfully: `pg-core/dialect.js`'s `migrate()` wraps **every pending migration file in one single `session.transaction(...)`** (not even one transaction per file) before executing any of their statements — confirmed by reading the installed library source directly, not assumed. Postgres itself rejects `CREATE INDEX CONCURRENTLY` inside any transaction block, so this fails regardless of which schema/migration-authoring choice is made. Real options, none implemented: **(a)** apply that one migration's SQL directly against the database outside `drizzle-kit migrate` (a small one-off script, similar in shape to `scripts/curriculum-import.ts`, executed with a plain non-transactional client) and manually record the matching row in `__drizzle_migrations` so `drizzle-kit migrate`/`db:verify` don't try to reapply it or flag drift — most correct, most manual; **(b)** keep accepting a locking `CREATE INDEX` for now (what unit 3 actually did for `users_username_lower_key`) and revisit only once a genuinely large table needs a new index — defers the tooling gap rather than closing it. Was: (2026-09-13, spec 20 unit 3) — Drizzle's `db:generate` has no built-in option for it, and a search of this codebase's 21-migration history found zero prior uses of `CONCURRENTLY` anywhere, so there's no established pattern to follow, and it's unverified whether `drizzle-kit migrate`'s transaction-per-file execution can even run a statement that must execute outside a transaction without a runner change. Shipped as an ordinary (locking) index creation instead, on the reasoning that the `users` table's actual row count at this stage of the beta makes the real lock risk negligible — but the underlying gap (no concurrent-index capability exists in this project's migration tooling at all) is real and will recur for every future index added to a populated table, not just this one. Worth a dedicated infrastructure unit: confirm whether `drizzle-kit migrate` supports a non-transactional statement, and if not, decide the mechanism (hand-written migration outside the generator, a split migration step, etc.) before a genuinely large table needs a new index.
