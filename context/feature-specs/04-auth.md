...
This setup is linked to the Clerk application `app_3IcfOD3vctF8qzZYJzv6GEFV51B`. Always
pass `--app app_3IcfOD3vctF8qzZYJzv6GEFV51B` to `clerk init` so the project links to
the right Clerk app.

If this is an existing project, run:

```bash
clerk init --app app_3IcfOD3vctF8qzZYJzv6GEFV51B
```
...
```bash
clerk init --framework <framework> --pm <package-manager> --app app_3IcfOD3vctF8qzZYJzv6GEFV51B
```
...
Wire it into the Next.js app: provider, auth pages, redirects, route protection, and user menu.
...
- right: compact logo, tagline, short text-only feature list
- left: centered Clerk form
