...
describe("parseJobMessage", () => {
  it("recognizes a native S3 ObjectCreated notification as a preview job", () => {
    const result = parseJobMessage(
      s3NotificationBody("polyglot-dev-imports", "imports/abc/source.csv"),
    );
  ...
  it("URL/form-decodes the S3 object key (S3 encodes spaces as '+', everything else percent-encoded)", () => {
    const result = parseJobMessage(
      s3NotificationBody("bucket", "imports/abc/my+file%20name.csv"),
    );
    expect(result).toEqual({
      kind: "preview",
      bucket: "bucket",
      key: "imports/abc/my file name.csv",
    });
  });
