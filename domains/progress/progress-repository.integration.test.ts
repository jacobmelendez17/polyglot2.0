...
describe("progress repository", () => {
  ...
  it("keeps progress for User A separate from User B", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, gatoId, languageId } =
        await seedTestFixtures(tx);
      expect(await hasItemProgress(tx, learnerId, gatoId)).toBe(true);
      expect(await hasItemProgress(tx, developerId, gatoId)).toBe(false);

      const developerProgress = await getUserProgressForLanguage(
        tx,
        developerId,
        languageId,
      );
  ...
  it("keeps progress for one language from affecting another", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await tx
        .insert(levels)
        .values({ languageId: otherLanguage.id, levelNumber: 1 });

      const learnerOtherLanguageProgress = await getUserProgressForLanguage(
        tx,
        learnerId,
        otherLanguage.id,
      );
      ...
      const learnerDefaultLanguageProgress = await getUserProgressForLanguage(
        tx,
        learnerId,
        languageId,
      );
      expect(
        learnerDefaultLanguageProgress.map((p) => p.learningItemId),
      ).toContain(gatoId);
    });
  });
  ...
  it("rejects a progress row whose language_id disagrees with its learning item's language", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const [otherLanguage] = await tx
        .insert(languages)
        .values({ code: "other-lang-2", slug: "other-lang-2", name: "Other 2" })
        .returning();
  ...
  it("finds due-review items via the composite index, without scanning unrelated users", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, gatoId, casaId, languageId } =
        await seedTestFixtures(tx);
      const now = new Date();
  ...
  it("never returns a due item from a different language, even for the same user (spec 09 §5/§23)", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const [otherLanguage] = await tx
        ...
        .values({
          code: "fr-FR",
          slug: "french-review-filter-test",
          name: "French",
        })
        .returning();
      ...
      const dueOther = await getDueReviewItems(
        tx,
        learnerId,
        otherLanguage!.id,
        now,
      );
      expect(dueOther.map((item) => item.learningItemId)).toEqual([
        otherItem!.id,
      ]);
    });
  });
  ...
  it("cascades a user's progress, notes, synonyms, and idempotency rows when the user is deleted", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const [unlockRow] = await tx
        ...
        .where(
          and(
            ...
            eq(userLevelProgress.levelId, level1Id),
          ),
        );
      ...
      const [idempotencyRow] = await tx
        ...
        .where(eq(idempotencyKeys.userId, learnerId));
...
describe("progress repository — review-completion mutations (spec 09 unit 4)", () => {
  ...
  it("applyItemProgressUpdate advances the counters/stage/version and returns null on a stale expectedVersion", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, gatoId, languageId } = await seedTestFixtures(tx);
      const before = await lockItemProgressForReview(tx, {
        userId: learnerId,
        learningItemId: gatoId,
        languageId,
      });
      ...
      expect(updated?.currentCorrectStreak).toBe(
        before!.currentCorrectStreak + 1,
      );
      expect(getStageIndex(updated!.highestSrsStageReached)).toBe(
        Math.max(
          getStageIndex(before!.highestSrsStageReached),
          getStageIndex("beginner_3"),
        ),
      );
  ...
  it("countLevelGatingItems counts published items only, never pending or archived ones", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const [level] = await tx
        .insert(levels)
        .values({ languageId, levelNumber: 71, name: "Gating fixture" })
        .returning();
  ...
  it("countUserItemsAtOrAboveStageInLevel only counts progress rows meeting the qualifying stage set", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, gatoId, casaId, languageId, level1Id } =
        await seedTestFixtures(tx);
      await tx
        .update(userItemProgress)
  ...
  it("unlockLevel is idempotent — a second call does not change the original unlockedAt", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const first = await unlockLevel(tx, {
        userId: learnerId,
        levelId: level2Id,
        now: new Date("2026-03-01T00:00:00Z"),
      });
      const second = await unlockLevel(tx, {
        userId: learnerId,
        levelId: level2Id,
  ...
  it("a genuine two-connection concurrent completion applies exactly once — the loser's stale expectedVersion is rejected, not silently reapplied", async () => {
    ...
    if (!original)
      throw new Error("expected the seeded gato progress row to exist");

    try {
      const attempt = () =>
        testDb.transaction(async (tx) => {
          await lockItemProgressForReview(tx, {
            userId: learnerId,
            learningItemId: gatoId,
            languageId,
          });
...
describe("getNextUpcomingReviewAt", () => {
  it("returns the earliest nextReviewAt strictly after now, ignoring already-due items", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, casaId, aguaId } =
        await seedTestFixtures(tx);
      ...
      await tx.insert(userItemProgress).values([
        ...
        }, // already due
        {
          ...
          languageId,
          srsStage: "beginner_1",
          nextReviewAt: new Date(now.getTime() + 3600_000),
        }, // +1h
      ]);

      expect(
        await getNextUpcomingReviewAt(tx, learnerId, languageId, now),
      ).toEqual(new Date(now.getTime() + 3600_000));
    });
  });
  ...
  it("returns null when nothing is scheduled after now", async () => {
    await withTestTransaction(async (tx) => {
      ...
      expect(
        await getNextUpcomingReviewAt(tx, learnerId, languageId, new Date()),
      ).toBeNull();
    });
  });
});
...
describe("getUpcomingReviewForecast", () => {
  it("returns items becoming due within the window, with their type, excluding already-due and out-of-window items", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId, casaId, grammarYId } =
        await seedTestFixtures(tx);
      const now = new Date("2026-01-01T00:00:00Z");
      ...
      await tx.insert(userItemProgress).values([
        ...
        },
        {
          ...
          languageId,
          srsStage: "beginner_1",
          nextReviewAt: inWindow,
        },
      ]);

      const forecast = await getUpcomingReviewForecast(
        tx,
        learnerId,
        languageId,
        { after: now, until: new Date(now.getTime() + 7 * 24 * 3600_000) },
      );
...
describe("countProgressForItems", () => {
  it("counts only items the user actually has a progress row for, and returns 0 for an empty id list", async () => {
    await withTestTransaction(async (tx) => {
      ...
      expect(
        await countProgressForItems(tx, learnerId, [gatoId, casaId, aguaId]),
      ).toBe(1);
...
describe("reconcileFluentSchedules (spec 20 Fluent Mode)", () => {
  it("enabling schedules a maintenance review anchored to fluentAt (not now), only for terminal Fluent items of the matching content type", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, casaId, grammarYId } =
        await seedTestFixtures(tx);
      ...
      await reconcileFluentSchedules(tx, {
        ...
        languageId,
        itemType: "vocabulary",
        fluentModeEnabled: true,
      });

      const casaProgress = await getItemProgress(tx, learnerId, casaId);
      expect(casaProgress?.nextReviewAt).toEqual(
  ...
  it("disabling nulls out only Fluent-stage items with a live schedule, of the matching content type", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await reconcileFluentSchedules(tx, {
        ...
        languageId,
        itemType: "vocabulary",
        fluentModeEnabled: false,
      });

      expect(
        (await getItemProgress(tx, learnerId, casaId))?.nextReviewAt,
      ).toBeNull();
      // grammarYId untouched — different content type.
      expect(
        (await getItemProgress(tx, learnerId, grammarYId))?.nextReviewAt,
      ).toEqual(scheduled);
    });
  });
  ...
  it("is idempotent — a second enable call makes no further change once a maintenance schedule is already set", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const afterFirst = (await getItemProgress(tx, learnerId, casaId))
        ?.nextReviewAt;
      ...
      const afterSecond = (await getItemProgress(tx, learnerId, casaId))
        ?.nextReviewAt;
...
describe("progress repository — Danger Zone Resets (spec 20 unit 21)", () => {
  describe("getResetCandidateItems", () => {
    it("returns only enrolled items of the requested content type, with the item's level number", async () => {
      await withTestTransaction(async (tx) => {
        const { learnerId, languageId, gatoId, grammarYId, casaId } =
          await seedTestFixtures(tx);
        ...
        const vocabularyCandidates = await getResetCandidateItems(
          tx,
          learnerId,
          languageId,
          "vocabulary",
        ...
        expect(
          vocabularyCandidates.map((item) => item.learningItemId).sort(),
        ).toEqual([casaId, gatoId].sort());
        expect(
          vocabularyCandidates.every(
            (item) => item.levelNumber === FIXTURE_LEVEL_NUMBER,
          ),
        ).toBe(true);

        const grammarCandidates = await getResetCandidateItems(
          tx,
          learnerId,
          languageId,
          "grammar",
        );
        expect(grammarCandidates.map((item) => item.learningItemId)).toEqual([
          grammarYId,
        ]);
      });
    });
    ...
    it("narrows to one CEFR band when given one", async () => {
      await withTestTransaction(async (tx) => {
        const { learnerId, languageId, gatoId, level1Id, level2Id, rojoId } =
          await seedTestFixtures(tx);
        ...
        await tx
          ...
          .set({ levelId: level2Id })
          .where(eq(learningItems.id, rojoId));
        await tx
          .update(levels)
          .set({ cefrLevel: "A1" })
          .where(eq(levels.id, level1Id));
        await tx
          .update(levels)
          .set({ cefrLevel: "A2" })
          .where(eq(levels.id, level2Id));
        await tx.insert(userItemProgress).values({
          userId: learnerId,
        ...
        const a1Candidates = await getResetCandidateItems(
          tx,
          learnerId,
          languageId,
          "vocabulary",
        ...
        expect(a1Candidates.map((item) => item.learningItemId)).toEqual([
          gatoId,
        ]);

        const a2Candidates = await getResetCandidateItems(
          tx,
          learnerId,
          languageId,
          "vocabulary",
        ...
        expect(a2Candidates.map((item) => item.learningItemId)).toEqual([
          rojoId,
        ]);
      });
    });
  ...
  describe("resetItemProgressToBeginner", () => {
    it("zeros every named SRS aggregate, schedules a fresh Beginner 1 review, and leaves everything else untouched", async () => {
      await withTestTransaction(async (tx) => {
        ...
        await resetItemProgressToBeginner(tx, {
          userId: learnerId,
          items: [
            { learningItemId: gatoId, levelNumber: FIXTURE_LEVEL_NUMBER },
          ],
          now,
        });
        ...
        expect(progress?.lastReviewedAt).toEqual(
        ...
        const [note] = await tx
          ...
          .where(
            and(
              ...
              eq(userNotes.learningItemId, gatoId),
            ),
          );
        ...
        const [synonym] = await tx
          ...
          .where(
            and(
              eq(userSynonyms.userId, learnerId),
              eq(userSynonyms.learningItemId, gatoId),
            ),
          );
    ...
    it("applies the Level 1-2 accelerated interval when the item's level is 1 or 2", async () => {
      await withTestTransaction(async (tx) => {
        ...
        await resetItemProgressToBeginner(tx, {
          userId: learnerId,
          items: [{ learningItemId: gatoId, levelNumber: 1 }],
          now,
        });
  ...
  describe("Reset to Level primitives", () => {
    it("getItemProgressAboveLevel finds only items whose level is strictly above the threshold", async () => {
      await withTestTransaction(async (tx) => {
        const { learnerId, languageId, rojoId, level2Id } =
          await seedTestFixtures(tx);
        ...
        const above = await getItemProgressAboveLevel(
          tx,
          learnerId,
          languageId,
          FIXTURE_LEVEL_NUMBER,
        ...
        const aboveEverything = await getItemProgressAboveLevel(
          tx,
          learnerId,
          languageId,
          FIXTURE_NEXT_LEVEL_NUMBER,
    ...
    it("deleteItemProgressByIds removes exactly the given rows and nothing else", async () => {
      await withTestTransaction(async (tx) => {
        const { learnerId, languageId, gatoId, rojoId } =
          await seedTestFixtures(tx);
        await tx.insert(userItemProgress).values({
          userId: learnerId,
          learningItemId: rojoId,
    ...
    it("deleteLevelUnlocksAboveLevel removes only unlocks for Levels above the threshold", async () => {
      await withTestTransaction(async (tx) => {
        const { learnerId, languageId, level1Id, level2Id } =
          await seedTestFixtures(tx);
        ...
        const unlocked = await getUnlockedLevels(tx, learnerId, languageId);
        expect(unlocked.map((progress) => progress.levelId)).toEqual([
          level1Id,
        ]);
      });
    });
