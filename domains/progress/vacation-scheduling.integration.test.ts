...
async function seedProgressFixture(
  ...
  const [language] = await tx
    ...
    .values({
      code: `es-V${suffix}`,
      slug: `spanish-vacation-${suffix}`,
      name: `Spanish (vacation ${suffix})`,
    })
    .returning();
  ...
  const [user] = await tx
    .insert(users)
    .values({
      clerkUserId: `vacation-sched-${suffix}`,
      role: "user",
...
describe("applyVacationSchedulingAdjustment", () => {
  it("matches calculateVacationAdjustedReview for a wait that began before the vacation, one mid-vacation, and one already overdue", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const vacationEndedAt = new Date(
        vacationStartedAt.getTime() + 10 * DAY_MS,
      );

      const preVacationWaitStart = new Date(
        vacationStartedAt.getTime() - 5 * DAY_MS,
      );
      const preVacationNextReview = new Date(
        vacationStartedAt.getTime() + 3 * DAY_MS,
      );

      const midVacationWaitStart = new Date(
        vacationStartedAt.getTime() + 5 * DAY_MS,
      );
      const midVacationNextReview = new Date(
        midVacationWaitStart.getTime() + 4 * HOUR_MS,
      );

      const overdueWaitStart = new Date(
        vacationStartedAt.getTime() - 20 * DAY_MS,
      );
      const overdueNextReview = new Date(
        vacationStartedAt.getTime() - 2 * DAY_MS,
      );

      const fixture = await seedProgressFixture(tx, [
        {
          learnedAt: preVacationWaitStart,
          lastReviewedAt: preVacationWaitStart,
          nextReviewAt: preVacationNextReview,
        },
        {
          learnedAt: midVacationWaitStart,
          lastReviewedAt: null,
          nextReviewAt: midVacationNextReview,
        },
        {
          learnedAt: overdueWaitStart,
          lastReviewedAt: overdueWaitStart,
          nextReviewAt: overdueNextReview,
        },
      ]);

      await applyVacationSchedulingAdjustment(
        tx,
        fixture.userId,
        vacationStartedAt,
        vacationEndedAt,
      ...
      const rows = await tx
        ...
        .where(
          inArray(userItemProgress.learningItemId, fixture.learningItemIds),
        );
      ...
      expect(byItemId.get(fixture.learningItemIds[0])?.nextReviewAt).toEqual(
        expectedPreVacation,
      );
      expect(byItemId.get(fixture.learningItemIds[1])?.nextReviewAt).toEqual(
        expectedMidVacation,
      );
      expect(byItemId.get(fixture.learningItemIds[2])?.nextReviewAt).toEqual(
        expectedOverdue,
      );

      // The pre-vacation case is also spec 20's own worked example directly.
      expect(expectedPreVacation).toEqual(
        new Date(vacationEndedAt.getTime() + 3 * DAY_MS),
      );
      // The mid-vacation case is spec 20's "4 hours after Vacation Mode ends" example.
      expect(expectedMidVacation).toEqual(
        new Date(vacationEndedAt.getTime() + 4 * HOUR_MS),
      );
    });
  });
  ...
  it("leaves a Fluent item with no scheduled review (next_review_at IS NULL) untouched", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const fixture = await seedProgressFixture(tx, [
        { learnedAt, lastReviewedAt: learnedAt, nextReviewAt: null },
      ]);
      ...
      const [row] = await tx
        .select()
        .from(userItemProgress)
        .where(eq(userItemProgress.learningItemId, fixture.learningItemIds[0]));
