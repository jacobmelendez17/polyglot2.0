...
async function findGhostRow(tx: Parameters<typeof recordSentenceMiss>[0], userId: string, learningItemId: string, sentenceId: string) {
  const [row] = await tx
    .select()
...
describe("recordSentenceMiss (spec 20 Ghost Reviews)", () => {
  ...
  it("Minimal only activates after the second miss of the same sentence", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const afterFirst = await findGhostRow(tx, learnerId, grammarYId, SENTENCE_Y_ID);
      expect(afterFirst).toMatchObject({ missCount: 1, ghostStage: null, activatedAt: null });

      const secondMissAt = new Date("2026-01-02T00:00:00Z");
      await recordSentenceMiss(tx, { ...input, now: secondMissAt });
      const afterSecond = await findGhostRow(tx, learnerId, grammarYId, SENTENCE_Y_ID);
  ...
  it("a further miss of an already-active Ghost is a no-op — the row is untouched", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await recordSentenceMiss(tx, { ...input, now: new Date("2026-06-01T00:00:00Z") });
...
describe("applyGhostAnswer (spec 20 Ghost SRS / Incorrect Ghost Answer)", () => {
  async function activateGhost(tx: Parameters<typeof recordSentenceMiss>[0], userId: string, languageId: string, learningItemId: string, sentenceId: string) {
    await recordSentenceMiss(tx, { userId, languageId, learningItemId, sentenceId, contentType: "vocabulary", mode: "on", now: NOW });
  ...
  it("a correct answer advances one Ghost stage and reschedules per the fixed Ghost SRS", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId } = await seedTestFixtures(tx);
      const ghost = await activateGhost(tx, learnerId, languageId, gatoId, SENTENCE_GATO_ID);

      const result = await applyGhostAnswer(tx, { userId: learnerId, ghostProgressId: ghost.id, isCorrect: true, now: new Date("2026-01-01T04:00:00Z") });

      expect(result).toMatchObject({ kind: "continuing", ghostProgress: { ghostStage: "ghost_2", nextReviewAt: new Date("2026-01-01T16:00:00Z") } });
    });
  });
  ...
  it("a correct answer on Ghost 4 completes the Ghost — nextReviewAt null, completedAt set", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId } = await seedTestFixtures(tx);
      const ghost = await activateGhost(tx, learnerId, languageId, gatoId, SENTENCE_GATO_ID);
      await tx.update(userSentenceGhostProgress).set({ ghostStage: "ghost_4" }).where(eq(userSentenceGhostProgress.id, ghost.id));

      const completedAt = new Date("2026-03-01T00:00:00Z");
      const result = await applyGhostAnswer(tx, { userId: learnerId, ghostProgressId: ghost.id, isCorrect: true, now: completedAt });

      expect(result).toMatchObject({ kind: "completed", ghostProgress: { ghostStage: "ghost_4", nextReviewAt: null, completedAt } });
    });
  });
  ...
  it("an incorrect answer resets to Ghost 1 regardless of which stage it was on", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId } = await seedTestFixtures(tx);
      const ghost = await activateGhost(tx, learnerId, languageId, gatoId, SENTENCE_GATO_ID);
      await tx.update(userSentenceGhostProgress).set({ ghostStage: "ghost_3" }).where(eq(userSentenceGhostProgress.id, ghost.id));

      const now = new Date("2026-02-01T00:00:00Z");
      const result = await applyGhostAnswer(tx, { userId: learnerId, ghostProgressId: ghost.id, isCorrect: false, now });

      expect(result).toMatchObject({ kind: "continuing", ghostProgress: { ghostStage: "ghost_1", nextReviewAt: new Date("2026-02-01T04:00:00Z") } });
    });
  });
  ...
  it("returns null for a Ghost that does not belong to this user", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const result = await applyGhostAnswer(tx, { userId: "00000000-0000-0000-0000-000000000099", ghostProgressId: ghost.id, isCorrect: true, now: NOW });
...
describe("getDueGhosts", () => {
  it("returns only Ghosts whose nextReviewAt has passed, excluding future and terminal (null) ones", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await recordSentenceMiss(tx, { userId: learnerId, languageId, learningItemId: gatoId, sentenceId: SENTENCE_GATO_ID, contentType: "vocabulary", mode: "on", now: new Date(now.getTime() - 5 * 60 * 60 * 1000) });
      // Not yet due: activated just now (4h out).
      await recordSentenceMiss(tx, { userId: learnerId, languageId, learningItemId: grammarYId, sentenceId: SENTENCE_Y_ID, contentType: "grammar", mode: "on", now });

      const due = await getDueGhosts(tx, learnerId, languageId, now);
      expect(due).toHaveLength(1);
      expect(due[0]).toMatchObject({ learningItemId: gatoId, sentenceId: SENTENCE_GATO_ID });
    });
  });
});
...
describe("applyGhostVacationSchedulingAdjustment (spec 20 Vacation and Ghosts)", () => {
  it("preserves the remaining interval across a vacation, the same rule as normal scheduled reviews", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await recordSentenceMiss(tx, { userId: learnerId, languageId, learningItemId: gatoId, sentenceId: SENTENCE_GATO_ID, contentType: "vocabulary", mode: "on", now: activatedAt });
      ...
      await applyGhostVacationSchedulingAdjustment(tx, learnerId, vacationStartedAt, vacationEndedAt);
