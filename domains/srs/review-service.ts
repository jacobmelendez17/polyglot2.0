 ...
 */
async function withAccountSettingsRateLimit<T>(
  userId: string,
  fn: () => Promise<T>,
): Promise<T> {
...
export async function updateGrammarReviewType(input: {
  userId: string;
  languageId: string;
  reviewType: ReviewType;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarReviewType(db, input),
  );
}

/** Spec 20 Reviews — Vocabulary Review Type. */
export async function updateVocabularyReviewType(input: {
  userId: string;
  languageId: string;
  reviewType: ReviewType;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularyReviewType(db, input),
  );
}

/** Spec 20 Review Hints — Grammar Hint Order. */
export async function updateGrammarHintOrder(input: {
  userId: string;
  languageId: string;
  hintOrder: HintOrder;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarHintOrder(db, input),
  );
}

/** Spec 20 Review Hints — Vocabulary Hint Order. */
export async function updateVocabularyHintOrder(input: {
  userId: string;
  languageId: string;
  hintOrder: HintOrder;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularyHintOrder(db, input),
  );
}

/** Spec 20 Review Hints — Grammar Hint Mode. */
export async function updateGrammarHintMode(input: {
  userId: string;
  languageId: string;
  hintMode: HintMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarHintMode(db, input),
  );
}

/** Spec 20 Review Hints — Vocabulary Hint Mode. */
export async function updateVocabularyHintMode(input: {
  userId: string;
  languageId: string;
  hintMode: HintMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularyHintMode(db, input),
  );
}

/** Spec 20 Review UI — any of the seven independent boolean toggles. */
export async function updateReviewUiToggle(input: {
  userId: string;
  languageId: string;
  ...
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveReviewUiToggle(db, input),
  );
}

/** Spec 20 Review UI — Undo Action. */
export async function updateUndoAction(input: {
  userId: string;
  languageId: string;
  undoAction: UndoAction;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveUndoAction(db, input),
  );
}

/** Spec 20 SRS Strictness — Grammar SRS Strictness. */
export async function updateGrammarSrsStrictness(input: {
  userId: string;
  languageId: string;
  srsStrictness: SrsStrictness;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarSrsStrictness(db, input),
  );
}

/** Spec 20 SRS Strictness — Vocabulary SRS Strictness. */
export async function updateVocabularySrsStrictness(input: {
  userId: string;
  languageId: string;
  srsStrictness: SrsStrictness;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularySrsStrictness(db, input),
  );
}

/** Spec 20 SRS Interval — Grammar SRS Interval. */
export async function updateGrammarSrsIntervalMode(input: {
  userId: string;
  languageId: string;
  srsIntervalMode: SrsIntervalMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarSrsIntervalMode(db, input),
  );
}

/** Spec 20 SRS Interval — Vocabulary SRS Interval. */
export async function updateVocabularySrsIntervalMode(input: {
  userId: string;
  languageId: string;
  srsIntervalMode: SrsIntervalMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularySrsIntervalMode(db, input),
  );
}

/** Spec 20 Review Queue Timing. */
export async function updateReviewQueueTiming(input: {
  userId: string;
  languageId: string;
  reviewQueueTiming: ReviewQueueTimingMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveReviewQueueTiming(db, input),
  );
}
 ...
 */
export async function updateGrammarFluentMode(input: {
  userId: string;
  languageId: string;
  fluentMode: boolean;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    db.transaction(async (tx) => {
      const updated = await preferenceRepository.saveGrammarFluentMode(
        tx,
        input,
      );
...
export async function updateVocabularyFluentMode(input: {
  userId: string;
  languageId: string;
  fluentMode: boolean;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    db.transaction(async (tx) => {
      const updated = await preferenceRepository.saveVocabularyFluentMode(
        tx,
        input,
      );
      await reconcileFluentSchedules(tx, {
 ...
 */
export async function updateGrammarGhostMode(input: {
  userId: string;
  languageId: string;
  ghostMode: GhostMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarGhostMode(db, input),
  );
}

/** Spec 20 Ghost Reviews — Vocabulary Ghost Reviews. See `updateGrammarGhostMode`. */
export async function updateVocabularyGhostMode(input: {
  userId: string;
  languageId: string;
  ghostMode: GhostMode;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularyGhostMode(db, input),
  );
}
...
export async function updateGrammarMinimumLeechStage(input: {
  userId: string;
  languageId: string;
  minimumLeechStage: SrsStage;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveGrammarMinimumLeechStage(db, input),
  );
}

/** Spec 20 Leeches — Vocabulary Minimum SRS for Leech. See `updateGrammarMinimumLeechStage`. */
export async function updateVocabularyMinimumLeechStage(input: {
  userId: string;
  languageId: string;
  minimumLeechStage: SrsStage;
}) {
  return withAccountSettingsRateLimit(input.userId, () =>
    preferenceRepository.saveVocabularyMinimumLeechStage(db, input),
  );
}
