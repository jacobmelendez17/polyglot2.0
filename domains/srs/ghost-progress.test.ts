...
describe("activateGhost", () => {
  it("schedules Ghost 1, 4 hours out (spec's own worked example)", () => {
    expect(activateGhost(NOW)).toEqual({
      ghostStage: "ghost_1",
...
describe("calculateGhostAnswerResult — correct answers advance through the fixed Ghost SRS", () => {
  ...
  it.each(cases)(
    "%s correct -> %s, scheduled per the fixed Ghost SRS",
    (stage, expectedStage, expectedNextReviewAt) => {
      const result = calculateGhostAnswerResult(stage, true, NOW);
      expect(result).toEqual({
        kind: "advanced",
        ghostStage: expectedStage,
  ...
  it('Ghost 4 correct -> completed ("Ghost is gone"), not a fifth stage', () => {
    expect(calculateGhostAnswerResult("ghost_4", true, NOW)).toEqual({
      kind: "completed",
    });
  });
});
...
describe("calculateGhostMissOutcome (spec 20 Ghost Review Settings)", () => {
  it("Off never creates or records anything", () => {
    const result = calculateGhostMissOutcome({
      mode: "off",
      existingMissCount: 0,
      existingGhostStage: null,
      now: NOW,
    });
  ...
  it("a further normal-review miss of an already-active Ghost is a no-op — only answering the Ghost review itself changes it", () => {
    const onResult = calculateGhostMissOutcome({
      mode: "on",
      existingMissCount: 1,
      existingGhostStage: "ghost_2",
      now: NOW,
    });
    expect(onResult).toEqual({ kind: "no_op" });

    const minimalResult = calculateGhostMissOutcome({
      mode: "minimal",
      existingMissCount: 3,
      existingGhostStage: "ghost_1",
      now: NOW,
    });
