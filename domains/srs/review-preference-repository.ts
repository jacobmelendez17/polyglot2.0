 ...
 */
export async function findReviewPreferences(
  db: DbClient,
  userId: string,
  languageId: string,
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(userReviewPreferences.userId, userId),
        eq(userReviewPreferences.languageId, languageId),
      ),
    )
    .limit(1);

  return row
    ? toReviewPreferences(row)
...
async function savePreferenceField<
  Column extends keyof typeof userReviewPreferences.$inferInsert,
>(
  db: DbClient,
  column: Column,
  input: {
    userId: string;
    languageId: string;
    value: (typeof userReviewPreferences.$inferInsert)[Column];
  },
): Promise<ReviewPreferences> {
  const [row] = await db
...
export function saveGrammarReviewType(
  db: DbClient,
  input: { userId: string; languageId: string; reviewType: ReviewType },
) {
  return savePreferenceField(db, "grammarReviewType", {
    ...input,
...
export function saveVocabularyReviewType(
  db: DbClient,
  input: { userId: string; languageId: string; reviewType: ReviewType },
) {
  return savePreferenceField(db, "vocabularyReviewType", {
    ...input,
    value: input.reviewType,
  });
}
...
export function saveGrammarHintOrder(
  db: DbClient,
  input: { userId: string; languageId: string; hintOrder: HintOrder },
) {
  return savePreferenceField(db, "grammarHintOrder", {
    ...input,
    value: input.hintOrder,
  });
}

/** Spec 20 Review Hints — Vocabulary Hint Order. Meaningful only under Vocabulary Hint Mode "more". */
export function saveVocabularyHintOrder(
  db: DbClient,
  input: { userId: string; languageId: string; hintOrder: HintOrder },
) {
  return savePreferenceField(db, "vocabularyHintOrder", {
    ...input,
    value: input.hintOrder,
  });
}
...
export function saveGrammarHintMode(
  db: DbClient,
  input: { userId: string; languageId: string; hintMode: HintMode },
) {
  return savePreferenceField(db, "grammarHintMode", {
    ...input,
    value: input.hintMode,
  });
}

/** Spec 20 Review Hints — Vocabulary Hint Mode. */
export function saveVocabularyHintMode(
  db: DbClient,
  input: { userId: string; languageId: string; hintMode: HintMode },
) {
  return savePreferenceField(db, "vocabularyHintMode", {
    ...input,
    value: input.hintMode,
  });
}
...
export function saveReviewUiToggle(
  ...
  return savePreferenceField(db, input.field, {
    userId: input.userId,
    languageId: input.languageId,
    value: input.value,
  });
...
export function saveUndoAction(
  db: DbClient,
  input: { userId: string; languageId: string; undoAction: UndoAction },
) {
  return savePreferenceField(db, "undoAction", {
    ...input,
    value: input.undoAction,
  });
}

/** Spec 20 SRS Strictness — Grammar SRS Strictness. */
export function saveGrammarSrsStrictness(
  db: DbClient,
  input: { userId: string; languageId: string; srsStrictness: SrsStrictness },
) {
  return savePreferenceField(db, "grammarSrsStrictness", {
    ...input,
    value: input.srsStrictness,
  });
}

/** Spec 20 SRS Strictness — Vocabulary SRS Strictness. */
export function saveVocabularySrsStrictness(
  db: DbClient,
  input: { userId: string; languageId: string; srsStrictness: SrsStrictness },
) {
  return savePreferenceField(db, "vocabularySrsStrictness", {
    ...input,
    value: input.srsStrictness,
  });
}
...
export function saveGrammarSrsIntervalMode(
  db: DbClient,
  input: {
    userId: string;
    languageId: string;
  ...
  return savePreferenceField(db, "grammarSrsIntervalMode", {
    ...input,
    value: input.srsIntervalMode,
  });
}

/** Spec 20 SRS Interval — Vocabulary SRS Interval. */
export function saveVocabularySrsIntervalMode(
  db: DbClient,
  input: {
    userId: string;
    languageId: string;
  ...
  return savePreferenceField(db, "vocabularySrsIntervalMode", {
    ...input,
    value: input.srsIntervalMode,
  });
}
...
export function saveReviewQueueTiming(
  db: DbClient,
  input: {
    userId: string;
    languageId: string;
  ...
  return savePreferenceField(db, "reviewQueueTiming", {
    ...input,
    value: input.reviewQueueTiming,
  });
}
 ...
 */
export function saveGrammarFluentMode(
  db: DbClient,
  input: { userId: string; languageId: string; fluentMode: boolean },
) {
  return savePreferenceField(db, "grammarFluentMode", {
    ...input,
    value: input.fluentMode,
  });
}

/** Spec 20 Fluent Mode — Vocabulary Fluent Mode. See `saveGrammarFluentMode`. */
export function saveVocabularyFluentMode(
  db: DbClient,
  input: { userId: string; languageId: string; fluentMode: boolean },
) {
  return savePreferenceField(db, "vocabularyFluentMode", {
    ...input,
    value: input.fluentMode,
  });
}
...
export function saveGrammarGhostMode(
  db: DbClient,
  input: { userId: string; languageId: string; ghostMode: GhostMode },
) {
  return savePreferenceField(db, "grammarGhostMode", {
    ...input,
    value: input.ghostMode,
  });
}

/** Spec 20 Ghost Reviews — Vocabulary Ghost Reviews. */
export function saveVocabularyGhostMode(
  db: DbClient,
  input: { userId: string; languageId: string; ghostMode: GhostMode },
) {
  return savePreferenceField(db, "vocabularyGhostMode", {
    ...input,
    value: input.ghostMode,
  });
}
...
export function saveGrammarMinimumLeechStage(
  db: DbClient,
  input: { userId: string; languageId: string; minimumLeechStage: SrsStage },
) {
  return savePreferenceField(db, "grammarMinimumLeechStage", {
    ...input,
    value: input.minimumLeechStage,
  });
}

/** Spec 20 Leeches — Vocabulary Minimum SRS for Leech. */
export function saveVocabularyMinimumLeechStage(
  db: DbClient,
  input: { userId: string; languageId: string; minimumLeechStage: SrsStage },
) {
  return savePreferenceField(db, "vocabularyMinimumLeechStage", {
    ...input,
    value: input.minimumLeechStage,
  });
}
