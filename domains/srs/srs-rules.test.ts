...
describe("calculateNextReview — Default schedule (level 3+, spec 20 SRS Interval's 'Default Schedule Change')", () => {
  it("resolves each standard interval correctly", () => {
    ...
    ).toEqual(new Date("2026-01-01T04:00:00Z"));
    expect(
      calculateNextReview({
        stage: "beginner_2",
        level: 3,
    ...
    ).toEqual(new Date("2026-01-01T08:00:00Z"));
    expect(
      calculateNextReview({
        stage: "beginner_3",
        level: 3,
  ...
  it("September 12 + 3 months = December 12 — spec 20's own worked example", () => {
    ...
    ).toEqual(new Date("2026-12-12T00:00:00Z"));
  });
...
describe("calculateNextReview — SRS Interval modes (spec 20's Level 3+ Standard Intervals table)", () => {
  ...
  it("Beginner 1, 2, and 4 stay fixed regardless of mode — the spec's own 'these remain fixed' list", () => {
    ...
    for (const mode of modes) {
      expect(
        calculateNextReview({ stage: "beginner_1", level: 3, mode, now: NOW }),
      ).toEqual(new Date("2026-01-01T04:00:00Z"));
      expect(
        calculateNextReview({ stage: "beginner_2", level: 3, mode, now: NOW }),
      ).toEqual(new Date("2026-01-01T08:00:00Z"));
      expect(
        calculateNextReview({ stage: "beginner_4", level: 3, mode, now: NOW }),
      ).toEqual(new Date("2026-01-03T00:00:00Z"));
    }
  });
});
...
describe("calculateNextReview — calendar-month arithmetic at a month boundary", () => {
  it("documents native Date#setUTCMonth rollover for a month with no equivalent day, rather than an invented clamp — no spec example covers this case, so this is the honest, unmodified JS behavior, verified directly against Node rather than hand-computed", () => {
    ...
    ).toEqual(new Date("2027-03-03T00:00:00Z"));
  });
});
...
describe("calculateNextReview — accelerated schedule (levels 1-2)", () => {
  ...
  it("falls back to the mode-selected standard schedule at Familiar 1 and beyond, even on an accelerated level", () => {
    const accelerated = calculateNextReview({
      stage: "familiar_1",
      level: 1,
      mode: "default",
      now: NOW,
    });
    const standard = calculateNextReview({
      stage: "familiar_1",
      level: 5,
      mode: "default",
      now: NOW,
  ...
  it("matches spec 20's own worked example: Level 1, Interval = Longest", () => {
    const start = NOW;
    const afterB1 = calculateNextReview({
      stage: "beginner_1",
      level: 1,
      mode: "longest",
      now: start,
    ...
    const afterB2 = calculateNextReview({
      stage: "beginner_2",
      level: 1,
      mode: "longest",
      now: afterB1,
    ...
    const afterB3 = calculateNextReview({
      stage: "beginner_3",
      level: 1,
      mode: "longest",
      now: afterB2,
    ...
    const afterB4 = calculateNextReview({
      stage: "beginner_4",
      level: 1,
      mode: "longest",
      now: afterB3,
    ...
    const afterF1 = calculateNextReview({
      stage: "familiar_1",
      level: 1,
      mode: "longest",
      now: afterB4,
    ...
    const afterF2 = calculateNextReview({
      stage: "familiar_2",
      level: 1,
      mode: "longest",
      now: afterF1,
    ...
    const afterInt = calculateNextReview({
      stage: "intermediate",
      level: 1,
      mode: "longest",
      now: afterF2,
    ...
    const afterMaster = calculateNextReview({
      stage: "master",
      level: 1,
      mode: "longest",
      now: afterInt,
  ...
  it("later levels (3+) use the standard schedule, not accelerated", () => {
    const level3 = calculateNextReview({
      stage: "beginner_1",
      level: 3,
      mode: "default",
      now: NOW,
    });
    const level1 = calculateNextReview({
      stage: "beginner_1",
      level: 1,
      mode: "default",
      now: NOW,
...
describe("calculateFluentMaintenanceReview (spec 20 Fluent Mode)", () => {
  it("matches the spec's own worked example: reached Fluent January 1 -> next review July 1, six calendar months later", () => {
    const result = calculateFluentMaintenanceReview(
...
describe("isReviewDue", () => {
  it("is due once now reaches nextReviewAt", () => {
    ...
    expect(
      isReviewDue({ nextReviewAt: new Date(NOW.getTime() + 1), now: NOW }),
    ).toBe(false);
    expect(
      isReviewDue({ nextReviewAt: new Date(NOW.getTime() - 1), now: NOW }),
    ).toBe(true);
