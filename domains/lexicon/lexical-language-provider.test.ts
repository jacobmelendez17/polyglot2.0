...
describe("spanishLexicalProvider.deriveDictionaryLookups", () => {
  it("derives the lemma from an article-composed display word", () => {
    ...
    expect(spanishLexicalProvider.deriveDictionaryLookups("el padre")).toEqual(["el padre", "padre"]);
    expect(spanishLexicalProvider.deriveDictionaryLookups("la madre")).toEqual(["la madre", "madre"]);
  });

  it("puts the exact expression first, so phrase matching is always attempted before article removal", () => {
    expect(spanishLexicalProvider.deriveDictionaryLookups("el padre")[0]).toBe("el padre");
  });

  it("handles every article in the set, and only those", () => {
    for (const article of ["el", "la", "los", "las", "un", "una", "unos", "unas"]) {
      expect(spanishLexicalProvider.deriveDictionaryLookups(`${article} cosa`)).toEqual([`${article} cosa`, "cosa"]);
    }
  });
  ...
  it("does not blindly strip the first token of every phrase", () => {
    ...
    expect(spanishLexicalProvider.deriveDictionaryLookups("buenos días")).toEqual(["buenos días"]);
    expect(spanishLexicalProvider.deriveDictionaryLookups("por favor")).toEqual(["por favor"]);
    expect(spanishLexicalProvider.deriveDictionaryLookups("de nada")).toEqual(["de nada"]);
  });
  ...
  it("treats the accented pronoun él as a different word from the article el", () => {
    ...
    expect(spanishLexicalProvider.deriveDictionaryLookups("él")).toEqual(["él"]);
    expect(spanishLexicalProvider.deriveDictionaryLookups("él padre")).toEqual(["él padre"]);
  });

  it("never reduces a bare article to nothing", () => {
    expect(spanishLexicalProvider.deriveDictionaryLookups("el")).toEqual(["el"]);
    expect(spanishLexicalProvider.deriveDictionaryLookups("las")).toEqual(["las"]);
  });

  it("strips only the leading article from a longer expression, keeping the rest whole", () => {
    expect(spanishLexicalProvider.deriveDictionaryLookups("el fin de semana")).toEqual([
      "el fin de semana",
      "fin de semana",
    ]);
  });
...
describe("spanishLexicalProvider.regionCodeForSourceLabel", () => {
  it("maps source usage labels onto region codes", () => {
    expect(spanishLexicalProvider.regionCodeForSourceLabel("Mexico")).toBe("es-MX");
    expect(spanishLexicalProvider.regionCodeForSourceLabel("Spain")).toBe("es-ES");
    expect(spanishLexicalProvider.regionCodeForSourceLabel("Argentina")).toBe("es-AR");
  });

  it("returns null for a label that carries no regional meaning", () => {
    expect(spanishLexicalProvider.regionCodeForSourceLabel("masculine")).toBeNull();
    expect(spanishLexicalProvider.regionCodeForSourceLabel("colloquial")).toBeNull();
  });
});
