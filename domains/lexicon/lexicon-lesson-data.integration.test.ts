...
describe("getConfirmedDictionaryDataForItems", () => {
  it("returns an empty map when no ids are given", async () => {
    await withTestTransaction(async (tx) => {
      expect(await getConfirmedDictionaryDataForItems(tx, [])).toEqual(
        new Map(),
      );
    });
  });
  ...
  it("returns the confirmed mapping's primary sense, preferred pronunciation, synonyms, variants, usage labels, regional evidence, and attribution", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await tx.insert(dictionaryRelations).values([
        {
          ...
          targetLemma: "can",
          normalizedTargetLemma: "can",
        },
        {
          dictionaryEntryId: entry.id,
          relationType: "alternative_form",
          targetLemma: "perros",
          normalizedTargetLemma: "perros",
        },
      ]);

      await tx.insert(dictionaryRegionalEvidence).values({
        dictionaryEntryId: entry.id,
        regionCode: "es-MX",
        status: "recognized",
        matchedForm: "perro",
      ...
      await tx.insert(vocabularySelectedSenses).values({
        vocabularyItemId: item.id,
        dictionarySenseId: primarySense.id,
        position: 0,
      });
      ...
      expect(data?.regionalEvidence).toEqual([
        {
          regionCode: "es-MX",
          status: "recognized",
          matchedForm: "perro",
      ...
      expect(data?.attribution?.attributionText).toBe(
        "From Test, CC BY-SA 4.0",
      );
    ...
    await withTestTransaction(async (tx) => {
      ...
      const [entry] = await tx
        ...
        .values({
          ...
          sourceId: source.id,
          lemma: "gata",
          normalizedLemma: "gata",
          partOfSpeech: "noun",
          sourceEntryKey: "gata#noun#0",
        })
        .returning();
