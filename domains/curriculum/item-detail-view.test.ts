...
describe("itemDetailSections", () => {
  it("hides Progress during a lesson (spec 18)", () => {
    expect(itemDetailSections("lesson")).toEqual([
      "info",
      "examples",
      "resources",
    ]);
    expect(itemDetailSections("page")).toEqual([
      "info",
      "examples",
      "progress",
      "resources",
...
describe("buildItemDetailView — vocabulary", () => {
  it("shows Gender and Register in Details, and Word Type with pronunciation", () => {
    const view = buildItemDetailView(
      vocabularySource({ register: "informal" }),
    );
    ...
    expect(view.pronunciation).toMatchObject({
      wordType: "noun",
      ipa: "/ˈɡa.to/",
      spokenText: "el gato",
    });
  ...
  it("shows N/A for a word with no grammatical gender, and an em dash for an unclassified register", () => {
    const view = buildItemDetailView(
      vocabularySource({ gender: null, register: null }),
    );
...
describe("buildItemDetailView — grammar", () => {
  it("titles the About card after the grammar point, preferring its longer title", () => {
    ...
    expect(
      buildItemDetailView(grammarSource({ title: "Ser vs. estar" })).about
        .title,
    ).toBe("About Ser vs. estar");
  });
  ...
  it("falls back to the legacy explanation only when no content blocks are authored", () => {
    expect(buildItemDetailView(grammarSource()).about.body).toBe(
      "Permanent states.",
    );
  ...
  it("orders content blocks by position, not by the order they arrived in", () => {
    ...
    expect(view.about.blocks.map((block) => block.id)).toEqual([
      "block-1",
      "block-2",
      "block-3",
    ]);
...
describe("buildItemDetailView — patterns of use", () => {
  ...
  const examples = [
    ...
    {
      id: "example-2",
      targetText: "Comes mucho.",
      translation: "You eat a lot.",
      patternId: "pattern-2",
    },
  ];
  ...
  it("groups examples under their pattern and keeps a pattern with no examples yet", () => {
    const view = buildItemDetailView(
      vocabularySource({ patterns, examples: [examples[0]] }),
    );

    expect(view.patterns.map((pattern) => pattern.label)).toEqual([
      "como",
      "comes",
    ]);
    expect(view.patterns[0].examples.map((example) => example.id)).toEqual([
      "example-1",
    ]);
  ...
  it("adds a General tab only when an example belongs to no pattern", () => {
    const withoutGeneral = buildItemDetailView(
      vocabularySource({ patterns, examples }),
    );
    expect(withoutGeneral.patterns.map((pattern) => pattern.id)).toEqual([
      "pattern-1",
      "pattern-2",
    ]);
    ...
    const withGeneral = buildItemDetailView(
      vocabularySource({
        ...
        examples: [
          ...
          {
            id: "example-3",
            targetText: "Comer es vivir.",
            translation: "To eat is to live.",
            patternId: null,
          },
        ],
      }),
    );
    expect(withGeneral.patterns.at(-1)).toMatchObject({
  ...
  it("lists every example in the Examples section regardless of pattern grouping", () => {
    ...
    expect(view.examples.map((example) => example.id)).toEqual([
      "example-1",
      "example-2",
    ]);
  });
...
describe("buildItemNavigation", () => {
  ...
  it("reports a 1-based position within its scope", () => {
    expect(buildItemNavigation(ids, "b", "Home & Basics")).toMatchObject({
      position: 2,
      total: 3,
  ...
  it("wraps in both directions (spec 18)", () => {
    expect(buildItemNavigation(ids, "a", "scope")).toMatchObject({
      previousItemId: "c",
      nextItemId: "b",
    });
    expect(buildItemNavigation(ids, "c", "scope")).toMatchObject({
      previousItemId: "b",
      nextItemId: "a",
    });
  });
