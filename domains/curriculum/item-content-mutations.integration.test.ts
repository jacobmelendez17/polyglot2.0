...
import { getGrammarContentBlocks, getItemResources } from "./curriculum-repository";
...
describe("grammar content block mutations", () => {
  it("appends each new block after the last", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "First." });
      await createGrammarContentBlock(tx, grammarYId, { type: "note", body: "Second." });
  ...
  it("clears the previous type's columns when a block changes type", async () => {
    await withTestTransaction(async (tx) => {
      const { grammarYId } = await seedTestFixtures(tx);
      const blockId = await createGrammarContentBlock(tx, grammarYId, { type: "example", targetText: "pan y agua", translation: "bread and water" });

      // The check constraint rejects a row carrying both shapes, so this
      // passing is itself the assertion that nothing was merged.
      await updateGrammarContentBlock(tx, blockId, { type: "text", body: "Joins two nouns." });
  ...
  it("reorders without ever violating the position constraint", async () => {
    await withTestTransaction(async (tx) => {
      const { grammarYId } = await seedTestFixtures(tx);
      const first = await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "First." });
      const second = await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "Second." });
      const third = await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "Third." });
      ...
      expect((await getGrammarContentBlocks(tx, grammarYId)).map((block) => block.id)).toEqual([third, first, second]);
    });
  });
  ...
  it("leaves the remaining blocks readable after a delete", async () => {
    await withTestTransaction(async (tx) => {
      const { grammarYId } = await seedTestFixtures(tx);
      const first = await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "First." });
      await createGrammarContentBlock(tx, grammarYId, { type: "text", body: "Second." });
...
describe("item resource mutations", () => {
  it("appends, updates, reorders, and deletes", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const first = await createItemResource(tx, gatoId, { label: "First", url: "https://example.invalid/a" });
      const second = await createItemResource(tx, gatoId, { label: "Second", url: "https://example.invalid/b" });
      ...
      expect((await getItemResources(tx, gatoId))[0]).toMatchObject({ label: "Renamed", url: "https://example.invalid/a" });

      await reorderItemResources(tx, gatoId, [second, first]);
      expect((await getItemResources(tx, gatoId)).map((resource) => resource.id)).toEqual([second, first]);
  ...
  it("updates only the field it is given", async () => {
    await withTestTransaction(async (tx) => {
      const { gatoId } = await seedTestFixtures(tx);
      const id = await createItemResource(tx, gatoId, { label: "Label", url: "https://example.invalid/a" });

      await updateItemResource(tx, id, { url: "https://example.invalid/b" });

      expect((await getItemResources(tx, gatoId))[0]).toMatchObject({ label: "Label", url: "https://example.invalid/b" });
    });
  });
});
