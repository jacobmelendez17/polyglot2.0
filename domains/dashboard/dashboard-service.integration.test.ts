...
async function seedIsolatedLearner(tx: DbClient): Promise<IsolatedLearner> {
  ...
  const [language] = await tx
    .insert(languages)
    .values({
      code: `fixture-dash-${unique}`,
      slug: `fixture-dash-${unique}`,
  ...
  const [learner] = await tx
    .insert(users)
    .values({ activeLanguageId: languageId, role: "user" })
    .returning();
...
describe("getDashboardData", () => {
  it("aggregates real lessons/reviews/level-progress data for a partially-progressed learner", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const data = await getDashboardData(tx, {
        userId: learnerId,
        languageId,
      });
      ...
      expect(
        data.forecast["24h"].every(
          (bucket) => bucket.vocabularyCount === 0 && bucket.grammarCount === 0,
        ),
      ).toBe(true);
      expect(
        data.reviewHistory["7d"].every((point) => point.completedCount === 0),
      ).toBe(true);
      expect(data.levelProgress.streak.every((day) => !day.isActive)).toBe(
        true,
      );
  ...
  it("reports the next upcoming review time when nothing is due yet, and reflects it in the forecast", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const totalForecastCount = data.forecast["24h"].reduce(
        (sum, bucket) => sum + bucket.vocabularyCount + bucket.grammarCount,
        0,
      );
  ...
  it("defaults to level 1 with zero totals when the language has no levels at all", async () => {
    await withTestTransaction(async (tx) => {
      const [emptyLanguage] = await tx
        .insert(languages)
        .values({ code: "xx-XX", slug: "empty-test-language", name: "Empty" })
        .returning();
      const [user] = await tx
        .insert(users)
        .values({ activeLanguageId: emptyLanguage.id })
        .returning();
