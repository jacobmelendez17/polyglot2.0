...
import { SANDBOX_SESSION_TTL_SECONDS, signSandboxGrant, verifySandboxGrant } from "./sandbox-session-token";
...
describe("sandbox view grant", () => {
  ...
  it("expires after the configured lifetime", async () => {
    const token = await signSandboxGrant(GRANT, NOW);
    expect(await verifySandboxGrant(token, NOW + SANDBOX_SESSION_TTL_SECONDS * 1000 - 1)).not.toBeNull();
    expect(await verifySandboxGrant(token, NOW + SANDBOX_SESSION_TTL_SECONDS * 1000)).toBeNull();
  });
  ...
  it("rejects a tampered payload", async () => {
    ...
    const forged = Buffer.from(
      JSON.stringify({ ...GRANT, sandboxUserId: "someone-else", issuedAt: NOW, expiresAt: NOW + 60_000 }),
      "utf8",
    ).toString("base64url");
