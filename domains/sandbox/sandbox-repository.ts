...
export async function findSandboxByOwner(
  db: DbClient,
  ownerUserId: string,
  ...
  const [row] = await db
    ...
    .from(users)
    .where(
      and(eq(users.sandboxOwnerUserId, ownerUserId), eq(users.isSandbox, true)),
    )
    .limit(1);
  return row
    ? { sandboxUserId: row.id, ownerUserId, languageId: row.activeLanguageId }
    : null;
}
...
export async function createSandboxForOwner(
  db: DbClient,
  {
    ownerUserId,
    languageId,
    level1Id,
  }: { ownerUserId: string; languageId: string; level1Id: string },
): Promise<SandboxAccount> {
  const [row] = await db
    .insert(users)
    .values({
      isSandbox: true,
      sandboxOwnerUserId: ownerUserId,
      activeLanguageId: languageId,
    })
    .returning({ id: users.id });
  const sandboxUserId = row!.id;
  await unlockLevel(db, {
...
export async function simulateLevel(
  db: DbClient,
  { sandboxUserId, levelId }: { sandboxUserId: string; levelId: string },
): Promise<void> {
...
export async function makeAllReviewsDue(
  db: DbClient,
  sandboxUserId: string,
): Promise<void> {
  await db
    ...
    .where(eq(userItemProgress.userId, sandboxUserId));
}
...
export async function resetSandbox(
  db: DbClient,
  { sandboxUserId, level1Id }: { sandboxUserId: string; level1Id: string },
): Promise<void> {
  await db
  ...
  await db
    ...
    .where(eq(userLevelProgress.userId, sandboxUserId));
  await unlockLevel(db, {
    userId: sandboxUserId,
    levelId: level1Id,
    now: new Date(),
...
export async function getSandboxSnapshot(
  ...
  const [levelRows, itemRows] = await Promise.all([
    db
      .select({
        ...
        unlockedAt: userLevelProgress.unlockedAt,
      })
  ...
  const timeOffsetSeconds = await getSandboxTimeOffset(
    db,
