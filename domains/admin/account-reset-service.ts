 ...
 */
export type ResetOwnAccountProgressServiceInput = { userId: string; languageId: string; idempotencyKey: string };

export async function resetOwnAccountProgress(db: DbClient, input: ResetOwnAccountProgressServiceInput): Promise<void> {
  return withIdempotency(
    db,
    { userId: input.userId, operation: "admin.account.reset-progress", key: input.idempotencyKey, payload: {} },
    async (tx) => {
      const level1 = await getLevelByLanguageAndNumber(tx, input.languageId, 1, { includeUnpublished: true });
      if (!level1) throw new AdminError("LEVEL_ONE_NOT_CONFIGURED");

      await resetAccountProgress(tx, { userId: input.userId, level1Id: level1.id });
