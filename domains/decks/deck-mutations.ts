 ...
 */
function assertOwnedPersonalDeck(deck: DeckRecord | null, userId: string): DeckRecord {
  if (!deck) throw new DeckError("DECK_NOT_FOUND");
  if (deck.kind !== "personal" || deck.ownerUserId !== userId) throw new DeckError("FORBIDDEN");
  return deck;
}
...
async function requireEligibleItems(
  ...
  const eligible = await filterEligibleLearningItemIds(db, { userId, languageId, learningItemIds });
  if (eligible.length !== learningItemIds.length) throw new DeckError("DECK_ITEM_NOT_ELIGIBLE");
}
...
export async function createPersonalDeck(db: DbClient, input: CreatePersonalDeckInput): Promise<{ deckId: string }> {
  ...
  return withIdempotency(
    db,
    {
      ...
      payload: { name: parsed.name, description: parsed.description, learningItemIds: parsed.learningItemIds },
    },
    async (tx) => {
...
export async function updatePersonalDeckDetails(db: DbClient, input: UpdatePersonalDeckDetailsInput): Promise<void> {
  ...
  await updateDeckFields(db, { deckId: parsed.deckId, name: parsed.name, description: parsed.description });
}
...
export async function addPersonalDeckItems(db: DbClient, input: AddPersonalDeckItemsInput): Promise<{ addedCount: number }> {
  const parsed = deckItemsSchema.parse(input);

  return db.transaction(async (tx) => {
    const deck = assertOwnedPersonalDeck(await getDeckRecordForUpdate(tx, parsed.deckId), input.userId);
...
export async function removePersonalDeckItem(db: DbClient, input: RemovePersonalDeckItemInput): Promise<void> {
  const parsed = deckItemSchema.parse(input);

  await db.transaction(async (tx) => {
    assertOwnedPersonalDeck(await getDeckRecordForUpdate(tx, parsed.deckId), input.userId);
...
export async function reorderPersonalDeckItems(db: DbClient, input: ReorderPersonalDeckItemsInput): Promise<void> {
 ...
 * permutation rewrite.
 */
export async function applyReorder(tx: DbClient, deckId: string, orderedLearningItemIds: string[]): Promise<void> {
  const currentIds = await getDeckItemIds(tx, deckId);
  const current = new Set(currentIds);
  const supplied = new Set(orderedLearningItemIds);

  const isValidSelection =
    supplied.size === orderedLearningItemIds.length && orderedLearningItemIds.every((id) => current.has(id));
...
export async function deletePersonalDeck(db: DbClient, input: DeletePersonalDeckInput): Promise<void> {
