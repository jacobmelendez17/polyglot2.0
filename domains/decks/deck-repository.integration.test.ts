...
describe("deck eligibility (spec 14)", () => {
  it("offers only items the learner has already learned", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const eligible = await getEligibleDeckItems(tx, {
        userId: learnerId,
        languageId,
        limit: 100,
      });
  ...
  it("rejects a personal deck built from an item the learner has not learned", async () => {
    await withTestTransaction(async (tx) => {
      ...
      expect(
        await tx.select().from(decks).where(eq(decks.ownerUserId, learnerId)),
      ).toHaveLength(0);
    });
  });
});
...
describe("personal deck mutations (spec 14)", () => {
  ...
  it("refuses to remove the final item, so a deck can never reach zero", async () => {
    await withTestTransaction(async (tx) => {
      ...
      expect(
        await tx.select().from(deckItems).where(eq(deckItems.deckId, deckId)),
      ).toHaveLength(1);
    });
  });
  ...
  it("reorders items and rejects an ordering containing an item the deck does not hold", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await reorderPersonalDeckItems(tx, {
        userId: learnerId,
        deckId,
        orderedLearningItemIds: [casaId, gatoId],
      });

      const deck = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
      expect(deck?.items.map((item) => item.learningItemId)).toEqual([
        casaId,
        gatoId,
      ]);
      ...
      await expect(
        reorderPersonalDeckItems(tx, {
          userId: learnerId,
          deckId,
          orderedLearningItemIds: [casaId, aguaId],
        }),
      ...
      await reorderPersonalDeckItems(tx, {
        userId: learnerId,
        deckId,
        orderedLearningItemIds: [gatoId, gatoId],
      });
      const afterDuplicate = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
      expect(afterDuplicate?.items.map((item) => item.learningItemId)).toEqual([
        gatoId,
        casaId,
      ]);
    });
  ...
  it("stays reorderable when the deck holds an item the learner can no longer see", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await tx
        ...
        .where(eq(userItemProgress.learningItemId, casaId));

      const visible = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
      expect(visible?.items.map((item) => item.learningItemId)).toEqual([
        gatoId,
      ]);
  ...
  it("deletes a deck and its membership without touching the curriculum items or SRS progress", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const before = await tx
        ...
        .where(eq(userItemProgress.learningItemId, gatoId));
      const { deckId } = await createPersonalDeck(tx, {
        userId: learnerId,
        languageId,
        name: "Temporary",
      ...
      expect(
        await tx.select().from(decks).where(eq(decks.id, deckId)),
      ).toHaveLength(0);
      expect(
        await tx.select().from(deckItems).where(eq(deckItems.deckId, deckId)),
      ).toHaveLength(0);
...
describe("deck ownership and Polyglot immutability (spec 14)", () => {
  it("refuses every personal-deck mutation from a user who does not own the deck", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await expect(
        deletePersonalDeck(tx, { userId: OTHER_USER_ID, deckId }),
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
      await expect(
        reorderPersonalDeckItems(tx, {
          userId: OTHER_USER_ID,
  ...
  it("refuses a learner's attempt to modify a Polyglot deck", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await expect(
        deletePersonalDeck(tx, { userId: learnerId, deckId }),
      ).rejects.toMatchObject({ code: "FORBIDDEN" });
      await expect(
        removePersonalDeckItem(tx, {
          userId: learnerId,
      ...
      const detail = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
  ...
  it("refuses an admin deck mutation aimed at a learner's personal deck", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, languageId, gatoId } =
        await seedTestFixtures(tx);
      const { deckId } = await createPersonalDeck(tx, {
        userId: learnerId,
        languageId,
...
describe("Polyglot deck availability (spec 14)", () => {
  it("hides a Level deck until its Level is unlocked, then shows every configured item", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, languageId, level2Id, rojoId } =
        await seedTestFixtures(tx);
      ...
      expect(
        (await listDecksForLearner(tx, { userId: learnerId, languageId })).map(
          (deck) => deck.id,
        ),
      ).not.toContain(deckId);
      expect(
        await getDeckForLearner(tx, { userId: learnerId, languageId, deckId }),
      ).toBeNull();
      ...
      expect(detail?.items.map((item) => item.learningItemId)).toEqual([
        rojoId,
      ]);
  ...
  it("shows a Theme deck immediately but only the items the learner has reached", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, languageId, gatoId, casaId } =
        await seedTestFixtures(tx);
      const { deckId } = await createPolyglotDeck(tx, {
        actorUserId: developerId,
        languageId,
      ...
      const before = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
      expect(before?.items.map((item) => item.learningItemId)).toEqual([
        gatoId,
      ]);
      ...
      const after = await getDeckForLearner(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
      expect(after?.items.map((item) => item.learningItemId)).toEqual([
        gatoId,
        casaId,
      ]);
    });
  ...
  it("counts only practicable items on the deck card", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, languageId, gatoId, casaId, grammarYId } =
        await seedTestFixtures(tx);
      ...
      const summary = (
        await listDecksForLearner(tx, { userId: learnerId, languageId })
      ).find((deck) => deck.name === "Mixed");
  ...
  it("lets an official deck reference published curriculum no learner has reached", async () => {
    await withTestTransaction(async (tx) => {
      const { developerId, languageId, gatoId, rojoId } =
        await seedTestFixtures(tx);
      const { deckId } = await createPolyglotDeck(tx, {
        actorUserId: developerId,
        languageId,
      ...
      const result = await addPolyglotDeckItems(tx, {
        actorUserId: developerId,
        deckId,
        learningItemIds: [rojoId],
      });
...
describe("deck practice never mutates learning state (spec 14)", () => {
  it("builds a session from the deck without changing any progress row", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const session = await startDeckPractice(tx, {
        userId: learnerId,
        languageId,
        deckId,
      });
  ...
  it("refuses to practice an item that is not practicable in this deck for this learner", async () => {
    await withTestTransaction(async (tx) => {
      ...
      expect(
        await isDeckItemPracticable(tx, {
          userId: learnerId,
          languageId,
          deckId,
          learningItemId: gatoId,
  ...
  it("refuses to start practice on a deck the learner cannot see", async () => {
    await withTestTransaction(async (tx) => {
      ...
      await expect(
        startDeckPractice(tx, { userId: learnerId, languageId, deckId }),
