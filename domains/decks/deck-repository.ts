...
function visibleDeckCondition(
  ...
  return and(
    ...
    or(
      ...
      and(
        eq(decks.kind, "polyglot"),
        eq(decks.availability, "level"),
        inArray(decks.gateLevelId, unlockedLevelIds(db, userId)),
      ),
    ),
  );
}
...
function emptyCounts(): DeckItemCounts {
  return {
    configured: { vocabulary: 0, grammar: 0 },
    learned: { vocabulary: 0, grammar: 0 },
  };
}
 ...
 */
async function getDeckItemCounts(
  db: DbClient,
  deckIds: string[],
  userId: string,
): Promise<Map<string, DeckItemCounts>> {
  ...
  const rows = await db
    .select({
      ...
      configured: count(),
      learned:
        sql<number>`count(*) filter (where ${userItemProgress.learningItemId} is not null)`.mapWith(
          Number,
        ),
    })
    .from(deckItems)
    ...
    .leftJoin(
      userItemProgress,
      and(
        eq(userItemProgress.learningItemId, deckItems.learningItemId),
        eq(userItemProgress.userId, userId),
      ),
    )
    .where(
      and(
        inArray(deckItems.deckId, deckIds),
...
export async function listDecksForLearner(
  ...
  const counts = await getDeckItemCounts(
    db,
...
async function getDeckItemRows(
  db: DbClient,
  {
    ...
    userId,
    learnedOnly,
  }: { deckId: string; userId: string; learnedOnly: boolean },
): Promise<DeckItemRow[]> {
  const rows = await db
    .select({
    ...
    .leftJoin(
      vocabularyItems,
      eq(vocabularyItems.learningItemId, deckItems.learningItemId),
    )
    .leftJoin(
      grammarItems,
      eq(grammarItems.learningItemId, deckItems.learningItemId),
    )
    .leftJoin(
      userItemProgress,
      and(
  ...
  return rows
    .map(toDeckItemRow)
    .filter((row): row is DeckItemRow => row !== null);
}
...
export async function getDeckForLearner(
  db: DbClient,
  {
    ...
    languageId,
    deckId,
  }: { userId: string; languageId: string; deckId: string },
  ...
  const [deck] = await db
    ...
    .from(decks)
    .where(
      and(eq(decks.id, deckId), visibleDeckCondition(db, userId, languageId)),
    )
    .limit(1);
  ...
  const [items, counts] = await Promise.all([
    getDeckItemRows(db, {
      deckId,
      userId,
      learnedOnly: deck.availability !== "level",
    }),
  ...
  return {
    ...
    contentType: deriveDeckContentType(
      (counts.get(deckId) ?? emptyCounts()).configured,
    ),
 ...
 */
export async function getDeckRecordForUpdate(
  db: DbClient,
  deckId: string,
): Promise<DeckRecord | null> {
...
async function selectDeckRecord(
  db: DbClient,
  deckId: string,
  forUpdate: boolean,
): Promise<DeckRecord | null> {
...
export async function getPublishedItemsForAdmin(
  db: DbClient,
  {
    ...
    search,
    limit,
  }: { languageId: string; search?: string; limit: number },
): Promise<DeckPickerItem[]> {
  const rows = await db
...
export async function filterPublishedLearningItemIds(
  db: DbClient,
  {
    languageId,
    learningItemIds,
  }: { languageId: string; learningItemIds: string[] },
): Promise<string[]> {
  if (learningItemIds.length === 0) return [];
  const rows = await db
    .select({ id: learningItems.id })
  ...
  const [row] = await db
    ...
    .where(
      and(
        ...
        or(
          eq(decks.availability, "level"),
          isNotNull(userItemProgress.learningItemId),
        ),
      ),
    )
    .limit(1);
...
export async function getDeckItemIds(
  db: DbClient,
  deckId: string,
): Promise<string[]> {
  const rows = await db
...
export async function insertDeckItems(
  db: DbClient,
  {
    ...
    languageId,
    learningItemIds,
  }: { deckId: string; languageId: string; learningItemIds: string[] },
): Promise<void> {
  if (learningItemIds.length === 0) return;
  const [existing] = await db
    .select({
      nextPosition:
        sql<number>`coalesce(max(${deckItems.position}), 0) + 1`.mapWith(
          Number,
        ),
    })
    .from(deckItems)
...
export async function updateDeckFields(
  db: DbClient,
  {
    ...
    name,
    description,
  }: { deckId: string; name: string; description: string | null },
): Promise<void> {
 ...
 */
export async function reorderDeckItems(
  db: DbClient,
  deckId: string,
  ...
  for (let i = 0; i < orderedLearningItemIds.length; i++) {
    await db
      ...
      .where(
        and(
          eq(deckItems.deckId, deckId),
          eq(deckItems.learningItemId, orderedLearningItemIds[i]!),
        ),
      );
  }
  for (let i = 0; i < orderedLearningItemIds.length; i++) {
...
export async function listPolyglotDecksForAdmin(
  ...
  (DeckRecord & { itemCount: number; gateLevelNumber: number | null })[]
> {
  const rows = await db
    .select({
      id: decks.id,
      ...
      itemCount:
        sql<number>`(select count(*) from ${deckItems} where ${deckItems.deckId} = ${decks.id})`.mapWith(
          Number,
        ),
    })
    .from(decks)
...
export async function getDeckItemsForAdmin(
  db: DbClient,
  deckId: string,
): Promise<DeckItemRow[]> {
  const rows = await db
