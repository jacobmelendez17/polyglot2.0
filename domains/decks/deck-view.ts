import type { DeckContentFilter, DeckContentType, DeckItemType, DeckSummary } from "./deck-types";
 ...
 */
export function deriveDeckContentType(counts: Record<DeckItemType, number>): DeckContentType | null {
...
export const DECK_CONTENT_FILTERS: readonly { value: DeckContentFilter; label: string }[] = [
  { value: "all", label: "All" },
...
const CONTENT_FILTER_VALUES = new Set<string>(DECK_CONTENT_FILTERS.map((option) => option.value));

/** Narrows an untrusted `?type=` search param; anything unrecognized falls back to the unfiltered default. */
export function parseDeckContentFilter(raw: string | undefined): DeckContentFilter {
  return raw !== undefined && CONTENT_FILTER_VALUES.has(raw) ? (raw as DeckContentFilter) : "all";
}
...
export function matchesDeckSearch(deck: Pick<DeckSummary, "name" | "description">, query: string): boolean {
 ...
 */
export function matchesDeckContentFilter(deck: Pick<DeckSummary, "contentType">, filter: DeckContentFilter): boolean {
  if (filter === "all") return true;
  return deck.contentType === filter;
}

export function filterDecks<T extends Pick<DeckSummary, "name" | "description" | "contentType">>(
  decks: T[],
  { search, contentFilter }: { search: string; contentFilter: DeckContentFilter },
): T[] {
  return decks.filter((deck) => matchesDeckSearch(deck, search) && matchesDeckContentFilter(deck, contentFilter));
}
 ...
 */
export const DECK_ACCENT_CLASSES: Record<DeckContentType | "none", { border: string; icon: string; iconBackground: string }> = {
  vocabulary: {
