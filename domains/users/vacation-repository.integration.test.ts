...
describe("vacation-repository", () => {
  ...
  it("startVacationPeriod creates an active period, and is idempotent on a repeat call", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const second = await startVacationPeriod(
        tx,
        user.id,
  ...
  it("endVacationPeriod closes the active period and is idempotent on a repeat call", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const secondClose = await endVacationPeriod(
        tx,
        user.id,
  ...
  it("getVacationPeriodsForUser returns every period, past and active, newest first", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const active = await startVacationPeriod(
        tx,
        user.id,
  ...
  it("a real concurrent enable only creates one active period — the database decides, not a check-then-insert race", async () => {
    ...
    try {
      ...
      expect(activePeriods.filter((row) => row.endedAt === null)).toHaveLength(
        1,
      );
    } finally {
      if (userId)
