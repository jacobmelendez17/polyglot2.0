...
function toVacationPeriod(
  row: typeof userVacationPeriods.$inferSelect,
): VacationPeriod {
  return {
    id: row.id,
    userId: row.userId,
 ...
 * here.
 */
export async function getVacationPeriodsForUser(
  db: DbClient,
  userId: string,
...
export async function findActiveVacationPeriod(
  db: DbClient,
  userId: string,
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(userVacationPeriods.userId, userId),
        isNull(userVacationPeriods.endedAt),
      ),
    )
    .orderBy(desc(userVacationPeriods.startedAt))
    .limit(1);
 ...
 */
export async function startVacationPeriod(
  db: DbClient,
  userId: string,
  startedAt: Date,
): Promise<VacationPeriod> {
 ...
 */
export async function endVacationPeriod(
  db: DbClient,
  userId: string,
  endedAt: Date,
): Promise<VacationPeriod | null> {
