...
describe("selectLessonBatch", () => {
  it("respects the configured batch size", () => {
    const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety" });
    expect(batch).toHaveLength(6);
  });

  it("returns an empty batch for an empty eligible set", () => {
    expect(selectLessonBatch({ eligibleItems: [], batchSize: 6, mode: "variety" })).toEqual([]);
  });

  it("does not let the client widen the batch beyond the eligible pool", () => {
    const batch = selectLessonBatch({ eligibleItems: [makeItem({ id: "only-item" })], batchSize: 6, mode: "variety" });
  ...
  it("teaches the current level only, never mixing a higher level into the same batch", () => {
    ...
    const batch = selectLessonBatch({ eligibleItems, batchSize: 6, mode: "variety" });
  ...
  describe("default_order mode", () => {
    it("teaches grammar first, then each vocabulary group in position order", () => {
      const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 20, mode: "default_order" });
    ...
    it("is a plain slice of the authored sequence — a short batch stops mid-sequence, not padded from elsewhere", () => {
      const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 3, mode: "default_order" });
      expect(batch.map((item) => item.id)).toEqual(["grammar-0", "grammar-1", "grammar-2"]);
    });
    ...
    it("continues into vocabulary once grammar is exhausted", () => {
      ...
      const batch = selectLessonBatch({ eligibleItems, batchSize: 3, mode: "default_order" });
      expect(batch.map((item) => item.id)).toEqual(["grammar-0", "numbers-0", "numbers-1"]);
    });

    it("ignores Grammar Placement entirely — grammar is always first regardless", () => {
      const batchFirst = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "default_order", grammarPlacement: "last" });
      const batchDefault = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "default_order" });
      expect(batchFirst.map((item) => item.id)).toEqual(batchDefault.map((item) => item.id));
  ...
  describe("choose_group mode", () => {
    ...
    it("never fills a short group from another group", () => {
      ...
      const batch = selectLessonBatch({ eligibleItems, batchSize: 8, mode: "choose_group", selectedThemeId: NUMBERS.id });
    ...
    it("selects nothing when no group has been chosen — the caller asks the learner instead", () => {
      expect(selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "choose_group", selectedThemeId: null })).toEqual([]);
    });
    ...
    it("still teaches grammar in its own curriculum order alongside the group, unaffected by Grammar Placement", () => {
      ...
      expect(batch.map((item) => item.type)).toEqual(["vocabulary", "vocabulary", "vocabulary", "vocabulary", "grammar", "grammar"]);
    ...
    it("scales the grammar share to the level, not to a fixed curriculum shape", () => {
      ...
      const heavyBatch = selectLessonBatch({ eligibleItems: grammarHeavy, batchSize: 6, mode: "choose_group", selectedThemeId: NUMBERS.id });
      ...
      const lightBatch = selectLessonBatch({ eligibleItems: vocabularyHeavy, batchSize: 6, mode: "choose_group", selectedThemeId: NUMBERS.id });
  ...
  describe("variety mode", () => {
    ...
    it("is deterministic — the same eligible curriculum always produces the same batch", () => {
      const first = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety" });
      const second = selectLessonBatch({ eligibleItems: [...makeLevel()].reverse(), batchSize: 6, mode: "variety" });
      expect(first.map((item) => item.id)).toEqual(second.map((item) => item.id));
    });

    describe("Grammar Placement", () => {
      it("First: grammar comes before vocabulary", () => {
        const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "first" });
        const grammarIndexes = batch.flatMap((item, index) => (item.type === "grammar" ? [index] : []));
        const vocabularyIndexes = batch.flatMap((item, index) => (item.type === "vocabulary" ? [index] : []));
        expect(Math.max(...grammarIndexes)).toBeLessThan(Math.min(...vocabularyIndexes));
      });

      it("Last: grammar comes after vocabulary", () => {
        const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "last" });
        const grammarIndexes = batch.flatMap((item, index) => (item.type === "grammar" ? [index] : []));
        const vocabularyIndexes = batch.flatMap((item, index) => (item.type === "vocabulary" ? [index] : []));
        expect(Math.min(...grammarIndexes)).toBeGreaterThan(Math.max(...vocabularyIndexes));
      });

      it("No Preference: grammar is neither forced first nor last — it's interleaved", () => {
        const batch = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "no_preference" });
        const grammarIndexes = batch.flatMap((item, index) => (item.type === "grammar" ? [index] : []));
        const vocabularyIndexes = batch.flatMap((item, index) => (item.type === "vocabulary" ? [index] : []));
        ...
        expect(Math.max(...grammarIndexes)).toBeGreaterThan(Math.min(...vocabularyIndexes));
        expect(Math.min(...grammarIndexes)).toBeLessThan(Math.max(...vocabularyIndexes));
      });

      it("No Preference never drops or duplicates an item relative to First/Last", () => {
        const first = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "first" });
        const noPreference = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "no_preference" });
        expect(new Set(noPreference.map((item) => item.id))).toEqual(new Set(first.map((item) => item.id)));
      });

      it("defaults to No Preference when unspecified", () => {
        const withDefault = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety" });
        const explicit = selectLessonBatch({ eligibleItems: makeLevel(), batchSize: 6, mode: "variety", grammarPlacement: "no_preference" });
        expect(withDefault.map((item) => item.id)).toEqual(explicit.map((item) => item.id));
      });
    });
  });
});
