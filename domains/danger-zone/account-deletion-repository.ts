...
function toAccountDeletionRequest(row: typeof accountDeletionRequests.$inferSelect): AccountDeletionRequest {
  return {
    id: row.id,
    userId: row.userId,
...
export async function getActiveDeletionRequest(db: DbClient, userId: string): Promise<AccountDeletionRequest | null> {
  const [row] = await db
    .select()
    .from(accountDeletionRequests)
    .where(and(eq(accountDeletionRequests.userId, userId), isNull(accountDeletionRequests.cancelledAt), isNull(accountDeletionRequests.completedAt)))
    .limit(1);
 ...
 */
export async function createDeletionRequest(db: DbClient, input: { userId: string; now: Date }): Promise<AccountDeletionRequest> {
  const existing = await getActiveDeletionRequest(db, input.userId);
  if (existing) return existing;

  const [row] = await db.insert(accountDeletionRequests).values({ userId: input.userId, requestedAt: input.now }).returning();
...
export async function confirmDeletionRequest(db: DbClient, input: { userId: string; now: Date; deleteAfter: Date }): Promise<AccountDeletionRequest | null> {
  const [row] = await db
    .update(accountDeletionRequests)
    .set({ confirmedAt: input.now, deleteAfter: input.deleteAfter, updatedAt: input.now })
    .where(
      and(
...
export async function cancelDeletionRequest(db: DbClient, input: { userId: string; now: Date }): Promise<AccountDeletionRequest | null> {
  const [row] = await db
    .update(accountDeletionRequests)
    .set({ cancelledAt: input.now, updatedAt: input.now })
    .where(and(eq(accountDeletionRequests.userId, input.userId), isNull(accountDeletionRequests.cancelledAt), isNull(accountDeletionRequests.completedAt)))
    .returning();
  return row ? toAccountDeletionRequest(row) : null;
}
...
export async function getDueDeletionRequests(db: DbClient, now: Date): Promise<AccountDeletionRequest[]> {
  const rows = await db
    .select()
    .from(accountDeletionRequests)
    .where(
...
export async function markDeletionRequestCompleted(db: DbClient, input: { requestId: string; now: Date }): Promise<void> {
  await db
    .update(accountDeletionRequests)
