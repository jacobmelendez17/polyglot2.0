...
import {
  deleteAllLearnerApplicationData,
  resetUserIdentityFields,
} from "./account-reset-repository";

export type ResetEntireAccountInput = {
  userId: string;
...
export async function resetEntireAccount(
  ...
  return withIdempotency(
    ...
    {
      userId: input.userId,
      operation: "danger-zone.reset-entire-account",
    ...
    async (tx) => {
      ...
      const level1 = await getLevelByLanguageAndNumber(tx, language.id, 1, {
      ...
      await resetUserIdentityFields(tx, {
        userId: input.userId,
