 ...
 */
async function seedFullAccountState(tx: Parameters<typeof seedTestFixtures>[0], input: { learnerId: string; languageId: string; gatoId: string; level2Id: string }) {
  ...
  await tx.insert(userReviewPreferences).values({ userId: learnerId, languageId, grammarGhostMode: "off" });
  await tx.insert(userNotificationPreferences).values({ userId: learnerId, newsUpdates: false });
  await tx.insert(userPreferences).values({ userId: learnerId, hideEnglishReviews: true });
  await tx.insert(decks).values({ languageId, kind: "personal", ownerUserId: learnerId, availability: "theme", name: "My deck" });
  await tx.insert(userVacationPeriods).values({ userId: learnerId, startedAt: new Date("2026-07-01T00:00:00Z"), endedAt: new Date("2026-07-05T00:00:00Z") });
  await tx.insert(userStreakAdjustments).values({ userId: learnerId, value: 20 });
  await tx.insert(userDismissedNotices).values({ userId: learnerId, noticeKey: "VACATION_LESSON_WARNING" });

  await tx
    .update(users)
    .set({ username: "old_handle", displayName: "Old Name", onboardingCompletedAt: new Date("2026-06-01T00:00:00Z"), timezone: "America/Phoenix" })
...
describe("resetEntireAccount (spec 20 Danger Zone)", () => {
  it("clears every learner-application-data table for this user", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId, level2Id } = await seedTestFixtures(tx);
      await seedFullAccountState(tx, { learnerId, languageId, gatoId, level2Id });

      await resetEntireAccount(tx, { userId: learnerId, idempotencyKey: crypto.randomUUID(), now: new Date("2026-08-10T00:00:00Z") });
      ...
      for (const table of tables) {
        const rows = await tx.select().from(table).where(eq(table.userId, learnerId));
        expect(rows, `expected ${table} to be empty for the reset learner`).toHaveLength(0);
      }

      const ownedDecks = await tx.select().from(decks).where(eq(decks.ownerUserId, learnerId));
  ...
  it("re-establishes exactly a freshly provisioned account's identity/onboarding fields, keeping Clerk identity and role", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId, level2Id } = await seedTestFixtures(tx);
      await seedFullAccountState(tx, { learnerId, languageId, gatoId, level2Id });
      const [before] = await tx.select().from(users).where(eq(users.id, learnerId));

      await resetEntireAccount(tx, { userId: learnerId, idempotencyKey: crypto.randomUUID(), now: new Date("2026-08-10T00:00:00Z") });

      const [after] = await tx.select().from(users).where(eq(users.id, learnerId));
  ...
  it("re-unlocks Level 1 and removes every other Level unlock — the same starting point a fresh account has", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, languageId, gatoId, level1Id, level2Id } = await seedTestFixtures(tx);
      await seedFullAccountState(tx, { learnerId, languageId, gatoId, level2Id });
  ...
  it("never touches another user's data", async () => {
    await withTestTransaction(async (tx) => {
      const { learnerId, developerId, languageId, gatoId, level2Id } = await seedTestFixtures(tx);
      await seedFullAccountState(tx, { learnerId, languageId, gatoId, level2Id });
      await tx.insert(userStreakAdjustments).values({ userId: developerId, value: 5 });
      await tx.insert(userDismissedNotices).values({ userId: developerId, noticeKey: "OTHER_WARNING" });

      await resetEntireAccount(tx, { userId: learnerId, idempotencyKey: crypto.randomUUID(), now: new Date("2026-08-10T00:00:00Z") });

      const developerStreak = await tx.select().from(userStreakAdjustments).where(eq(userStreakAdjustments.userId, developerId));
      expect(developerStreak).toHaveLength(1);
      const developerNotices = await tx.select().from(userDismissedNotices).where(eq(userDismissedNotices.userId, developerId));
  ...
  it("is idempotent — retrying with the same key does not re-run the wipe or fail against an already-empty account", async () => {
    await withTestTransaction(async (tx) => {
      ...
      const first = await resetEntireAccount(tx, { userId: learnerId, idempotencyKey, now });
      const second = await resetEntireAccount(tx, { userId: learnerId, idempotencyKey, now });
