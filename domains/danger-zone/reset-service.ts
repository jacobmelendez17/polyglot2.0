...
import { deleteGhostProgressForContentType, deleteGhostProgressForLearningItems } from "@/domains/srs/ghost-repository";
 ...
 */
export async function resetContentTypeReviews(db: DbClient, input: ResetContentTypeReviewsInput): Promise<ContentTypeResetResult> {
  ...
  return withIdempotency(
    db,
    {
      ...
      payload: { languageId: input.languageId, contentType: input.contentType, target: input.target },
    },
    async (tx) => {
      if (input.target === "ghost") {
        const affectedItemCount = await deleteGhostProgressForContentType(tx, input.userId, input.languageId, input.contentType);
        return { affectedItemCount };
      }

      const cefrLevel = isCefrLevel(input.target) ? input.target : undefined;
      const candidates = await getResetCandidateItems(tx, input.userId, input.languageId, input.contentType, cefrLevel);

      let targets = candidates;
      if (input.target === "leech") {
        const preferences = await findReviewPreferences(tx, input.userId, input.languageId);
        const minimumLeechStage =
          input.contentType === "grammar" ? preferences.grammarMinimumLeechStage : preferences.vocabularyMinimumLeechStage;
      ...
      await resetItemProgressToBeginner(tx, {
        userId: input.userId,
        items: targets.map((item) => ({ learningItemId: item.learningItemId, levelNumber: item.levelNumber })),
        now,
      });
 ...
 */
export async function resetToLevel(db: DbClient, input: ResetToLevelInput): Promise<ContentTypeResetResult> {
  return withIdempotency(
    db,
    {
      userId: input.userId,
      ...
      key: input.idempotencyKey,
      payload: { languageId: input.languageId, targetLevelNumber: input.targetLevelNumber },
    },
    async (tx) => {
      ...
      if (!unlockedLevelNumbers.includes(input.targetLevelNumber) || input.targetLevelNumber >= currentLevelNumber) {
        throw new AppError("RESET_TARGET_INVALID", "Choose an earlier, already-unlocked Level to reset to.");
      }

      const itemsAbove = await getItemProgressAboveLevel(tx, input.userId, input.languageId, input.targetLevelNumber);
      ...
      if (itemIdsAbove.length > 0) {
        ...
        await deleteGhostProgressForLearningItems(tx, input.userId, itemIdsAbove);
        await deleteItemProgressByIds(tx, input.userId, itemIdsAbove);
      }
      await deleteLevelUnlocksAboveLevel(tx, input.userId, input.languageId, input.targetLevelNumber);
