...
function toStreakAdjustment(
  row: typeof userStreakAdjustments.$inferSelect,
): StreakAdjustment {
  return {
    id: row.id,
    userId: row.userId,
 ...
 */
export async function insertStreakAdjustment(
  db: DbClient,
  input: { userId: string; value: number; now: Date },
): Promise<StreakAdjustment> {
  const [row] = await db
...
export async function getLatestStreakAdjustment(
  db: DbClient,
  userId: string,
): Promise<StreakAdjustment | null> {
  const [row] = await db
    .select()
