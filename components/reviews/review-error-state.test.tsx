...
describe("ReviewErrorState", () => {
  it("states plainly that progress was not changed", () => {
    ...
    expect(
      screen.getByText(/Your SRS progress was not changed/),
    ).toBeInTheDocument();
  });
  ...
  it("offers a retry for a recoverable error", () => {
    render(
      <ReviewErrorState
        error={{ code: "RATE_LIMITED", message: "Please slow down." }}
      />,
    );
    expect(
  ...
  it("offers a link back to reviews for a non-recoverable error, instead of a dead retry button", () => {
    render(
      <ReviewErrorState
        error={{ code: "INVALID_REVIEW_STATE", message: "Session invalid." }}
      />,
    );
    expect(
    ...
    expect(
      screen.getByRole("link", { name: /back to reviews/i }),
