...
describe("ReviewHint", () => {
  it("renders nothing for hide", () => {
    const { container } = render(
      <ReviewHint hint={{ mode: "hide" }} autoExpand={false} />,
    );
  ...
  it("renders nothing for hint mode when the item has no nuance content", () => {
    const { container } = render(
      <ReviewHint hint={{ mode: "hint", nuance: null }} autoExpand={false} />,
    );
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("hint mode: reveals the nuance only after clicking Show Hint", async () => {
    ...
    render(
      <ReviewHint
        hint={{ mode: "hint", nuance: "Used for pets or strays." }}
        autoExpand={false}
      />,
    );

    expect(
      screen.queryByText("Used for pets or strays."),
  ...
  it("show mode: reveals the translation only after clicking Show Meaning", async () => {
    ...
    render(
      <ReviewHint
        hint={{ mode: "show", translation: "cat" }}
        autoExpand={false}
      />,
    );
  ...
  it("always_show_nuance: renders nothing when there is no nuance content", () => {
    const { container } = render(
      <ReviewHint
        hint={{ mode: "always_show_nuance", nuance: null }}
        autoExpand={false}
      />,
    );
    expect(container).toBeEmptyDOMElement();
  ...
  it("more mode: skips a step with no content instead of a dead button", () => {
    ...
    render(<ReviewHint hint={hint} autoExpand={false} />);
    expect(
      screen.getByRole("button", { name: "Show Meaning" }),
    ).toBeInTheDocument();
  });
