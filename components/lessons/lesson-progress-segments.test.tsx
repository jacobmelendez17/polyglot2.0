...
const ITEMS: ProgressSegmentItem[] = [
  ...
  { itemId: "b", itemType: "grammar", state: "current", stateLabel: "current" },
  {
    ...
    itemType: "vocabulary",
    state: "not-started",
    stateLabel: "not viewed",
  },
  {
    itemId: "d",
    itemType: "vocabulary",
    state: "partial",
    stateLabel: "partially satisfied",
  },
];
...
describe("LessonProgressSegments", () => {
  it("renders one segment per lesson item with accessible labels", () => {
    ...
    expect(
      screen.getByLabelText("Vocabulary item 1 of 4, viewed"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Grammar item 2 of 4, current"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Vocabulary item 3 of 4, not viewed"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Vocabulary item 4 of 4, partially satisfied"),
    ).toBeInTheDocument();
  });
