...
type PracticeAction =
  ...
  | {
      type: "CLASSIFY";
      classification: DeckPracticeClassification;
  ...
  switch (action.type) {
    ...
    case "ANSWERED":
      return {
        ...
        questionsCorrect:
          state.questionsCorrect + (action.feedback.kind === "correct" ? 1 : 0),
      };
    case "ADVANCE":
    ...
    case "CLASSIFY":
      return advance(
        {
          ...state,
          classifications: [...state.classifications, action.classification],
        },
 ...
 */
export function DeckPracticeView({
  deckId,
  deckName,
  itemCount,
  questions,
  characterHelpers,
}: DeckPracticeViewProps) {
  ...
  const isLastForItem = useMemo(() => {
    ...
    questions.forEach((question, index) =>
      lastIndexByItem.set(question.learningItemId, index),
    );
    return questions.map(
      (question, index) =>
        lastIndexByItem.get(question.learningItemId) === index,
    );
  }, [questions]);
  ...
  if (state.phase === "intro") {
    return (
      <DeckPracticeIntro
        ...
        onToggleKnowDontKnow={(enabled) =>
          dispatch({ type: "TOGGLE_KNOW_DONT_KNOW", enabled })
        }
  ...
  if (state.phase === "complete" || !question) {
    return (
      <DeckPracticeComplete
        ...
        summary={
          state.knowDontKnowEnabled
            ? summarizeDeckPractice(state.classifications)
            : null
        }
      />
    );
  ...
  const awaitingClassification =
    awaitingAdvance && state.knowDontKnowEnabled && isLastForItem[state.index];
  const progressPercent = Math.round((state.index / questions.length) * 100);
  const accuracyPercent =
    state.questionsAttempted === 0
      ? null
      : Math.round((state.questionsCorrect / state.questionsAttempted) * 100);
    ...
    <div className="mx-auto flex min-h-svh max-w-2xl flex-col px-4 py-6">
      <div className="flex items-center gap-3">
        ...
        <p className="shrink-0 text-xs whitespace-nowrap text-muted-foreground">
          {questions.length - state.index} left
      ...
      <div className="flex flex-1 flex-col items-center justify-center gap-8 py-12">
        ...
        <PracticeAnswerField
          ...
          onAdvance={() =>
            dispatch({ type: "ADVANCE", questionCount: questions.length })
          }
