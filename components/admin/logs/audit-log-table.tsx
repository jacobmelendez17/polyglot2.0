      ...
      <table className="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody className="divide-y divide-border">
          {events.map((event) => {
            ...
            const hasDetail = event.beforeData !== null || event.afterData !== null;
            return (
              <Fragment key={event.id}>
                <tr className="align-top">
                  <td className="whitespace-nowrap px-3 py-2 text-muted-foreground">{formatTimestamp(event.createdAt)}</td>
                  <td className="px-3 py-2">{actorNamesById[event.actorUserId] ?? `${event.actorUserId.slice(0, 8)}…`}</td>
                  <td className="px-3 py-2 font-mono text-xs">{event.action}</td>
                  <td className="px-3 py-2">
                    {event.resourceType}
                    {event.resourceId ? <span className="text-muted-foreground"> · {event.resourceId.slice(0, 8)}…</span> : null}
                  </td>
                    ...
                    <td colSpan={6} className="bg-muted/30 px-3 py-3">
                      <div className="grid gap-3 sm:grid-cols-2">
                        <div>
                          ...
                          <pre className="overflow-x-auto rounded-md bg-card p-2 text-xs">{JSON.stringify(event.beforeData, null, 2) ?? "null"}</pre>
                        </div>
                        <div>
                          <p className="mb-1 text-xs font-medium text-muted-foreground">After</p>
                          <pre className="overflow-x-auto rounded-md bg-card p-2 text-xs">{JSON.stringify(event.afterData, null, 2) ?? "null"}</pre>
                        </div>
                      </div>
                    </td>
                  </tr>
