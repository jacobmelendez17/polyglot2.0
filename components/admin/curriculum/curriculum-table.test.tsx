...
describe("CurriculumTable", () => {
  it("renders each item's type, item, meaning, level, group, and status", () => {
    render(<CurriculumTable items={[item({})]} selectedIds={new Set()} onToggleItem={noop} onToggleAll={noop} />);
  ...
  it("links each row's Edit action to that item's editor", () => {
    render(<CurriculumTable items={[item({})]} selectedIds={new Set()} onToggleItem={noop} onToggleAll={noop} />);
    expect(screen.getByRole("link", { name: "Edit" })).toHaveAttribute("href", "/admin/curriculum/items/item-1");
  });
  ...
  it("shows a designed empty state, not a blank table, when nothing matches", () => {
    render(<CurriculumTable items={[]} selectedIds={new Set()} onToggleItem={noop} onToggleAll={noop} />);
  ...
  it("reflects which rows are selected, including the header checkbox when every row is", () => {
    render(
      <CurriculumTable
        items={[item({ id: "item-1", itemLabel: "gato" }), item({ id: "item-2", itemLabel: "casa" })]}
    ...
    expect(screen.getByRole("checkbox", { name: "Select casa" })).not.toBeChecked();
    expect(screen.getByRole("checkbox", { name: "Select all items on this page" })).not.toBeChecked();
  });
  ...
  it("calls onToggleItem with that row's ID when its checkbox is clicked", async () => {
    ...
    render(
      <CurriculumTable items={[item({ id: "item-1", itemLabel: "gato" })]} selectedIds={new Set()} onToggleItem={onToggleItem} onToggleAll={noop} />,
    );
  ...
  it("calls onToggleAll when the header checkbox is clicked", async () => {
    ...
    render(<CurriculumTable items={[item({})]} selectedIds={new Set()} onToggleItem={noop} onToggleAll={onToggleAll} />);
