...
import {
  itemExampleAction,
  seedUsageContextsAction,
  usageContextAction,
} from "@/app/(admin)/admin/curriculum/actions";

export type UsageContextValue = {
  id: string;
  label: string;
  note: string | null;
  position: number;
 ...
 */
export function UsageContextEditor({
  learningItemId,
  itemType,
  contexts,
  examples,
  canSeedFromDictionary,
}: UsageContextEditorProps) {
  ...
  function seed() {
    ...
    startTransition(async () => {
      ...
      setNotice(
        result.data.reason ??
          `Added ${result.data.created} tab${result.data.created === 1 ? "" : "s"} from the dictionary.`,
      );
      router.refresh();
    });
  }
  ...
  function moveContext(index: number, direction: -1 | 1) {
    ...
    run(() =>
      usageContextAction({
        learningItemId,
        idempotencyKey: key(),
        mutation: { kind: "reorder", learningItemId, orderedIds: order },
      }),
    );
  }

  const examplesFor = (contextId: string | null) =>
    examples.filter((example) => example.usageContextId === contextId);

  return (
    <section className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-heading text-lg font-semibold text-foreground">
          Patterns of use &amp; examples
        </h2>
      ...
      {itemType === "vocabulary" && !canSeedFromDictionary ? (
        <p className="text-sm text-muted-foreground">
          Confirm a dictionary match below and the word&apos;s forms can fill
          these tabs for you. You can always write them by hand.
        </p>
      ) : null}
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                aria-label={`Move ${context.label} earlier`}
                disabled={isPending || index === 0}
                onClick={() => moveContext(index, -1)}
              >
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                aria-label={`Move ${context.label} later`}
                disabled={isPending || index === contexts.length - 1}
                onClick={() => moveContext(index, 1)}
              >
              ...
              <Button
                ...
                onClick={() =>
                  run(
                    ...
                    "Tab removed — its examples moved to General.",
                  )
                }
              >
              ...
              onDelete={(exampleId) =>
                run(() =>
                  itemExampleAction({
                    learningItemId,
                    idempotencyKey: key(),
                    mutation: { kind: "delete", exampleId },
                  }),
                )
              }
            />
            ...
            onClick={() => {
              ...
              run(() =>
                usageContextAction({
                  learningItemId,
                  idempotencyKey: key(),
                  mutation: { kind: "create", learningItemId, label },
                }),
              );
            }}
          >
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        {contexts.length > 0 ? (
          <label className="text-sm">
            ...
            <Select
              value={newExample.contextId}
              onValueChange={(value) =>
                setNewExample((previous) => ({ ...previous, contextId: value }))
              }
            >
            ...
            run(() =>
              itemExampleAction({
                ...
                mutation: {
                  ...
                  usageContextId: contextId === GENERAL ? null : contextId,
                },
              }),
            );
          }}
