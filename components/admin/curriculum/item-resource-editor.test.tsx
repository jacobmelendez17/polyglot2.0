...
describe("ItemResourceEditor", () => {
  ...
  it("sends a fresh idempotency key with every mutation", async () => {
    const user = userEvent.setup();
    render(
      <ItemResourceEditor learningItemId="item-1" resources={resources} />,
    );

    await user.click(
      screen.getByRole("button", { name: "Delete Conjugation table" }),
    );
    await user.click(screen.getByRole("button", { name: "Delete Video" }));

    const [first, second] = mockedAction.mock.calls.map(
  ...
  it("reorders by sending the full new order, not a single moved id", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Move Video earlier" }),
    );

    expect(mockedAction).toHaveBeenCalledWith(
      expect.objectContaining({
        mutation: { kind: "reorder", orderedIds: ["resource-2", "resource-1"] },
      }),
    );
  });
  ...
  it("cannot move the first resource up or the last one down", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Move Conjugation table earlier" }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: "Move Video later" }),
    ).toBeDisabled();
  });
  ...
  it("surfaces a rejected mutation instead of appearing to succeed", async () => {
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(
      "Resource links must start with http:// or https://",
    );
  });
