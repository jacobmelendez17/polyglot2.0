...
import {
  VocabularyEditor,
  type ResolvedVocabularyFieldInfo,
  type VocabularyEditorValue,
...
function grammarFieldsPayload(g: GrammarEditorValue) {
  return {
    ...
    requiredQuestions: g.requiredDirections.map(
 ...
 * client-side).
 */
export function CurriculumItemForm({
  languageId,
  levels,
  groups,
  existing,
  ...
  const [vocab, setVocab] = useState<VocabularyEditorValue>(
    existing?.vocabulary ?? EMPTY_VOCAB,
  );
  ...
  const [grammar, setGrammar] = useState<GrammarEditorValue>(
      ...
      const fieldsPayload =
        type === "vocabulary"
          ? {
              ...
              fields: vocabularyFieldsPayload(vocab),
            }
          : { type: "grammar" as const, fields: grammarFieldsPayload(grammar) };
          ...
          setDuplicateCandidates(
            (
              result.error.details as {
                candidates?: typeof duplicateCandidates;
              }
            )?.candidates ?? [],
          );
          return;
          ...
          <div className="mt-3 flex gap-2">
            ...
            {duplicateCandidates[0] ? (
              <Button
                ...
                size="sm"
                onClick={() =>
                  handleSubmit(duplicateCandidates[0]!.learningItemId)
                }
                disabled={isPending}
              >
