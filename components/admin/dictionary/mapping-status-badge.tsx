import { Check, CircleHelp, CircleSlash, Lock, TriangleAlert } from "lucide-react";

import { MATCH_CONFIDENCE_LABELS, MATCH_STATUS_LABELS } from "@/domains/lexicon";
import type { DictionaryMatchConfidence, DictionaryMatchStatus } from "@/domains/lexicon";
import { cn } from "@/lib/utils";

const STATUS_CONFIG: Record<DictionaryMatchStatus, { icon: typeof Check; className: string }> = {
  auto_matched: { icon: Check, className: "text-state-success bg-state-success/15" },
  manual: { icon: Lock, className: "text-state-success bg-state-success/15" },
  review_required: { icon: TriangleAlert, className: "text-state-warning bg-state-warning/15" },
  unmatched: { icon: CircleSlash, className: "text-muted-foreground bg-muted" },
  source_data_not_imported: { icon: CircleHelp, className: "text-muted-foreground bg-muted/50" },
};
...
export function MappingStatusBadge({
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium", className)}>
      ...
      {confidence ? <span className="font-normal opacity-80">· {MATCH_CONFIDENCE_LABELS[confidence]}</span> : null}
    </span>
  );
}
