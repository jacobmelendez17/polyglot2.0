...
import { updateGrammarFluentModeAction, updateVocabularyFluentModeAction } from "@/app/(app)/settings/reviews/actions";
...
export function FluentModeToggle({ contentType, initialValue }: FluentModeToggleProps) {
  const action = contentType === "grammar" ? updateGrammarFluentModeAction : updateVocabularyFluentModeAction;
  return (
    <InlineToggleSettingField
      label={contentType === "grammar" ? "Fluent Mode — Grammar" : "Fluent Mode — Vocabulary"}
      ...
      onSave={async (fluentMode) => {
        ...
        return result.ok ? { ok: true, value: result.data.fluentMode } : { ok: false, message: result.error.message };
      }}
    />
  );
}
