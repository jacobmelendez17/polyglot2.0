...
const OPTIONS: { value: SrsStage; label: string }[] = SRS_STAGE_ORDER.map(
  (stage) => ({ value: stage, label: SRS_STAGE_LABELS[stage] }),
);
...
export function MinimumLeechStageSelect({
  contentType,
  initialValue,
}: MinimumLeechStageSelectProps) {
  const action =
    contentType === "grammar"
      ? updateGrammarMinimumLeechStageAction
      : updateVocabularyMinimumLeechStageAction;
