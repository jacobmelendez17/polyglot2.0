...
describe("InlineTextSettingField", () => {
  it("shows Add with a placeholder when no value exists yet", () => {
    render(<InlineTextSettingField label="Name" initialValue={null} onSave={vi.fn()} />);
  ...
  it("shows the current value with an Edit action when one exists", () => {
    render(<InlineTextSettingField label="Name" initialValue="Jacob Melendez" onSave={vi.fn()} />);
  ...
  it("saves a new value and shows Saved on success", async () => {
    const onSave = vi.fn().mockResolvedValueOnce({ ok: true, value: "New Name" });
    const user = userEvent.setup();
    render(<InlineTextSettingField label="Name" initialValue="Old Name" onSave={onSave} />);
  ...
  it("shows the server error and keeps editing open on failure, without pretending the change saved", async () => {
    const onSave = vi.fn().mockResolvedValueOnce({ ok: false, message: "That value isn't valid." });
    const user = userEvent.setup();
    render(<InlineTextSettingField label="Name" initialValue="Old Name" onSave={onSave} />);
    ...
    expect(await screen.findByText("That value isn't valid.")).toBeInTheDocument();
