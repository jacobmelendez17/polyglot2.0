 ...
 */
export function ProgressEmailToggle({ initialValue }: ProgressEmailToggleProps) {
  return (
    <InlineToggleSettingField
      ...
      onSave={async (progressEmail) => {
        const result = await updateNotificationPreferencesAction({ progressEmail });
        return result.ok ? { ok: true, value: result.data.progressEmail } : { ok: false, message: result.error.message };
      }}
    />
  );
}
