...
export function AboutDefinition({ about, languageCode }: AboutDefinitionProps) {
  ...
  return (
    <div className="rounded-xl bg-muted/30 p-4 ring-1 ring-foreground/5 sm:p-5">
      <h3 className="font-heading text-lg font-semibold text-foreground">{about.title}</h3>

      <div className="mt-3 flex flex-col gap-4">
        {about.body ? <p className="text-base leading-relaxed text-foreground">{about.body}</p> : null}
        ...
        {about.dictionarySenses.length > 0 ? (
          <div className="rounded-lg bg-card p-3 ring-1 ring-foreground/10">
            <h4 className="text-xs font-medium text-muted-foreground">Dictionary senses</h4>
            ...
            {about.attribution ? <p className="mt-2 text-xs text-muted-foreground">{about.attribution}</p> : null}
          </div>
        ) : null}

        {hasContent ? null : <p className="text-sm text-muted-foreground">No explanation has been written for this item yet.</p>}
      </div>
    </div>
  );
}
