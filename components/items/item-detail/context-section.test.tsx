...
describe("ContextSection", () => {
  it("lays out patterns beside the selected pattern's examples", () => {
    ...
    expect(
      screen.getByText("first-person singular present"),
    ).toBeInTheDocument();
  ...
  it("switches the examples when another pattern is selected", async () => {
    ...
    expect(screen.getByRole("tab", { name: "comes" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
  ...
  it("still offers a pattern with no examples written for it yet", async () => {
    ...
    expect(
      screen.getByText("No examples for this pattern yet."),
    ).toBeInTheDocument();
  });
  ...
  it("moves between patterns with the arrow keys", async () => {
    ...
    expect(screen.getByRole("tab", { name: "come" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
  ...
  it("keeps only the selected pattern in the tab order", () => {
    ...
    expect(screen.getByRole("tab", { name: "como" })).toHaveAttribute(
      "tabindex",
      "0",
    );
    expect(screen.getByRole("tab", { name: "comes" })).toHaveAttribute(
  ...
  it("renders nothing at all when the item has no patterns", () => {
    const { container } = render(
      <ContextSection patterns={[]} languageCode="es-MX" />,
    );
  ...
  it("offers audio for each example", () => {
    render(<ContextSection patterns={patterns} languageCode="es-MX" />);
    expect(
      screen.getByRole("button", { name: /Como pan\./ }),
    ).toBeInTheDocument();
  });
});
