...
function about(
  overrides: Partial<ItemDetailAboutView> = {},
): ItemDetailAboutView {
  return {
    title: "Definition",
    body: null,
...
describe("AboutDefinition", () => {
  ...
  it("says an item has no explanation rather than rendering an empty card", () => {
    ...
    expect(
      screen.getByText("No explanation has been written for this item yet."),
    ).toBeInTheDocument();
  });
        ...
        about={about({
          ...
          blocks: [
            ...
            },
            {
              ...
              position: 3,
              type: "note",
              body: "Not to be confused with estar.",
            },
          ],
        })}
        languageCode="es-MX"
...
describe("ExamplesSection", () => {
  it("shows each sentence, its translation, and an audio control", () => {
    ...
    expect(
      screen.getByRole("button", { name: /El gato duerme\./ }),
    ).toBeInTheDocument();
  });
  ...
  it("says an item has no examples rather than rendering an empty list", () => {
    ...
    expect(
      screen.getByText("No example sentences for this item yet."),
    ).toBeInTheDocument();
  });
});
