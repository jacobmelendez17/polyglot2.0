...
export function ProgressSection({
  ...
  const unlockDate = levelUnlockedAt
    ? formatAbsoluteDate(levelUnlockedAt, timeZone)
    : EMPTY_FIELD;
    ...
    return (
      <div className="flex flex-col gap-4">
        <p className="text-base text-muted-foreground">
          Not yet studied — your progress appears here once you learn this item
          in a lesson.
        </p>
  ...
  return (
    <div className="flex flex-col gap-6">
      <dl className="grid grid-cols-2 gap-x-4 gap-y-4 sm:grid-cols-3 lg:grid-cols-4">
        ...
        <Metric label="Retired Date">
          {progress.fluentAt
            ? formatAbsoluteDate(progress.fluentAt, timeZone)
            : EMPTY_FIELD}
        </Metric>
        {/*
  ...
  return (
    ...
    >
      <p className="text-sm text-muted-foreground">
        Stage visualization coming soon
      </p>
    </div>
  );
}
