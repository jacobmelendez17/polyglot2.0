...
describe("ItemDetailTabs", () => {
  it("marks the active section without claiming to be a tablist", () => {
    render(
      <ItemDetailTabs
        sections={itemDetailSections("page")}
    ...
    expect(screen.getByRole("button", { name: "Info" })).not.toHaveAttribute(
      "aria-current",
    );
  ...
  it("omits Progress in lesson mode", () => {
    render(
      <ItemDetailTabs
        sections={itemDetailSections("lesson")}
        activeSection="info"
        onSelect={() => {}}
        variant="full"
      />,
...
describe("ResourcesSection", () => {
  it("opens each resource externally and says so in its accessible name", () => {
    ...
    const link = screen.getByRole("link", {
      name: "Conjugation table (opens in a new tab)",
    });
  ...
  it("says an item has no resources rather than rendering an empty list", () => {
    ...
    expect(
      screen.getByText("No additional resources for this item yet."),
    ).toBeInTheDocument();
  });
});
