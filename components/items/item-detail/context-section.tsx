...
export function ContextSection({
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLDivElement>) {
    ...
    const nextIndex =
      (index + (forward ? 1 : -1) + patterns.length) % patterns.length;
      ...
      <div className="grid gap-6 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
        <div>
          ...
          <div
            ...
            aria-label="Pattern of use"
            onKeyDown={handleKeyDown}
            className="mt-3 flex flex-wrap gap-1.5 sm:flex-col"
          >
        ...
        <div
          ...
          role="tabpanel"
          aria-labelledby={`context-pattern-${active.id}`}
          tabIndex={0}
        >
          <h3 className="font-heading text-base font-semibold text-foreground">
            Common Combinations
          ...
          {active.examples.length > 0 ? (
            ...
            <p className="mt-3 text-base text-muted-foreground">
              No examples for this pattern yet.
            </p>
          )}
        </div>
      </div>
