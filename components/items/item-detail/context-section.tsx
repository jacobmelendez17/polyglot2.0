  ...
  return (
    <div className="rounded-xl bg-muted/30 p-4 ring-1 ring-foreground/5 sm:p-5">
      <div className="grid gap-6 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
        <div>
          <h3 className="font-heading text-base font-semibold text-foreground">Pattern of Use</h3>
            ...
            {patterns.map((pattern) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "cursor-pointer rounded-lg px-3 py-1.5 text-left text-base transition-colors",
        ...
        <div id={`context-panel-${active.id}`} role="tabpanel" aria-labelledby={`context-pattern-${active.id}`} tabIndex={0}>
          <h3 className="font-heading text-base font-semibold text-foreground">Common Combinations</h3>
          {active.note ? <p className="mt-1 text-sm text-muted-foreground">{active.note}</p> : null}
          ...
          {active.examples.length > 0 ? (
            ...
            <p className="mt-3 text-base text-muted-foreground">No examples for this pattern yet.</p>
          )}
        </div>
      </div>
    </div>
