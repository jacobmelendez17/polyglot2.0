...
export const FONT_SCALES = ["small", "default", "large", "extra_large"] as const;
 ...
 */
export function parseAppearanceSettings(candidate: unknown): AppearanceSettings {
  if (typeof candidate !== "object" || candidate === null) return DEFAULT_APPEARANCE_SETTINGS;
  ...
  return {
    ...
    fontFamily: isFontFamily(value.fontFamily) ? value.fontFamily : DEFAULT_FONT_FAMILY,
    fontScale: isFontScale(value.fontScale) ? value.fontScale : DEFAULT_FONT_SCALE,
    colorBlindAssistance: typeof value.colorBlindAssistance === "boolean" ? value.colorBlindAssistance : DEFAULT_COLOR_BLIND_ASSISTANCE,
  };
}
...
export function resolvesToDark(theme: Theme, prefersDarkMediaQuery: boolean): boolean {
